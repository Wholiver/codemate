...
import { type Session } from "@codemate-ai/sdk/v2/client"
import {
  childSessionOnPath,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("codemate://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("codemate://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("codemate://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("codemate://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("codemate://open-project")).toBeUndefined()
    expect(parseDeepLink("codemate://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "codemate://open-project?directory=/a",
      "codemate://other?directory=/b",
      "codemate://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("codemate://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("codemate://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("codemate://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("codemate://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "codemate://new-session?directory=/a",
      "codemate://open-project?directory=/b",
      "codemate://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __codemate__: {
        deepLinks: ["codemate://open-project?directory=/a"],
      },
    } as unknown as Window & { __codemate__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["codemate://open-project?directory=/a"])
