...
const managedConfigDir = process.env.codemate_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "codemate.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "codemate.json") {
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "codemate.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "codemate.jsonc")
...
test("ignores legacy tui keys in codemate config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://codemate.ai/config.json",
  ...
  try {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "codemate.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["codemate_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { codemate: { options: { apiKey: "{env:codemate_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["codemate"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["codemate_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["codemate_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "codemate.json"), "{ invalid json }")
    },
  })
  await provideTestInstance({
    directory: tmp.path,
...
test("loads config from .codemate directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const codemateDir = path.join(dir, ".codemate")
      await fs.mkdir(codemateDir, { recursive: true })
      const agentDir = path.join(codemateDir, "agent")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".codemate", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .codemate/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const codemateDir = path.join(dir, ".codemate")
      await fs.mkdir(codemateDir, { recursive: true })

      const agentsDir = path.join(codemateDir, "agents")
...
test("loads commands from .codemate/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const codemateDir = path.join(dir, ".codemate")
      await fs.mkdir(codemateDir, { recursive: true })

      const commandDir = path.join(codemateDir, "command")
...
test("loads commands from .codemate/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const codemateDir = path.join(dir, ".codemate")
      await fs.mkdir(codemateDir, { recursive: true })

      const commandsDir = path.join(codemateDir, "commands")
...
test("does not try to install dependencies in read-only codemate_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.codemate_CONFIG_DIR
  process.env.codemate_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.codemate_CONFIG_DIR
    else process.env.codemate_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable codemate_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "codemate.json"),
        JSON.stringify({ $schema: "https://codemate.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .codemate config
      const projectDir = path.join(dir, "project")
      const codemateDir = path.join(projectDir, ".codemate")
      await fs.mkdir(codemateDir, { recursive: true })
      ...
      // Local .codemate config with different plugins
      await Filesystem.write(
        path.join(codemateDir, "codemate.json"),
        JSON.stringify({
          $schema: "https://codemate.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .codemate config with some overlapping plugins
      await Filesystem.write(
        path.join(codemateDir, "codemate.json"),
        JSON.stringify({
          $schema: "https://codemate.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".codemate")
...
// Note: preload.ts sets codemate_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .codemate config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .codemate directory config enables it
      const codemateDir = path.join(dir, ".codemate")
      await fs.mkdir(codemateDir, { recursive: true })
      await Filesystem.write(
        path.join(codemateDir, "codemate.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/codemate")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/codemate")
    ...
    if (urlStr.includes(".well-known/codemate")) {
      ...
      return Promise.resolve(
        new Response(
          JSON.stringify({
            remote_config: {
              url: "https://config.example.com/codemate.json",
              headers: {
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(wellknownFetchedUrl).toBe("https://example.com/.well-known/codemate")
            expect(remoteFetchedUrl).toBe("https://config.example.com/codemate.json")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "codemate.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-codemate@2.4.3", file)).toBe("oh-my-codemate@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-codemate@2.4.3", "file:///project/.codemate/plugin/oh-my-codemate.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.codemate/plugin/demo.ts", "file:///project/.codemate/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.codemate/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const codemateDir = path.join(projectDir, ".codemate")
        const pluginDir = path.join(codemateDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("codemate_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["codemate_DISABLE_PROJECT_CONFIG"]
    process.env["codemate_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["codemate_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["codemate_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .codemate/ directories when flag is set", async () => {
    const originalEnv = process.env["codemate_DISABLE_PROJECT_CONFIG"]
    process.env["codemate_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .codemate directory with a command
          const codemateDir = path.join(dir, ".codemate", "command")
          await fs.mkdir(codemateDir, { recursive: true })
          await Filesystem.write(path.join(codemateDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await WithInstance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .codemate should NOT be in directories list
          const hasProjectcodemate = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectcodemate).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["codemate_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["codemate_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["codemate_CONFIG_DIR"]
      process.env["codemate_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["codemate_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["codemate_CONFIG_DIR"]
      } else {
        process.env["codemate_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("codemate_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["codemate_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["codemate_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["codemate_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["codemate_CONFIG_DIR"] = configDirTmp.path

      await WithInstance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from codemate_CONFIG_DIR, not project
...
describe("codemate_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in codemate_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["codemate_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["codemate_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://codemate.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["codemate_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["codemate_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in codemate_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["codemate_CONFIG_CONTENT"]
  ...
  const config = ConfigParse.effectSchema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "codemate Managed",
          PayloadIdentifier: "ai.codemate.managed.test",
          PayloadType: "ai.codemate.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.effectSchema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://codemate.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://codemate.ai/config.json")
})
