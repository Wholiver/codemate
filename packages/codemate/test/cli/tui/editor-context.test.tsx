...
const originalcodematePort = process.env.codemate_EDITOR_SSE_PORT

afterEach(() => {
  process.env.CLAUDE_CODE_SSE_PORT = originalClaudePort
  process.env.codemate_EDITOR_SSE_PORT = originalcodematePort
})
...
test("useEditorContext reconnect switches editor server by session directory", async () => {
  ...
  process.env.codemate_EDITOR_SSE_PORT = undefined
...
test("useEditorContext connects with codemate_EDITOR_SSE_PORT", async () => {
  await using tmp = await tmpdir()
  process.env.CLAUDE_CODE_SSE_PORT = undefined
  process.env.codemate_EDITOR_SSE_PORT = "4020"
