...
import { codemateClient, type GlobalEvent } from "@codemate-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<codemateClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<codemateClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<codemateClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<codemateClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<codemateClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://codemate.test"),
...
function sdk(
  input: {
    ...
    subscribe?: codemateClient["event"]["subscribe"]
    globalEvent?: codemateClient["global"]["event"]
    promptAsync?: codemateClient["session"]["promptAsync"]
    status?: codemateClient["session"]["status"]
    messages?: codemateClient["session"]["messages"]
    children?: codemateClient["session"]["children"]
    permissions?: codemateClient["permission"]["list"]
    questions?: codemateClient["question"]["list"]
  } = {},
) {
  const client = new codemateClient()

  const subscribe: codemateClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: codemateClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: codemateClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: codemateClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: codemateClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: codemateClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: codemateClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: codemateClient["question"]["list"] = input.questions ?? (() => ok([]))
