...
- `config?: Partial<Config.Info>` - Write an `codemate.json` config file
...
- Directories are created in the system temp folder with prefix `codemate-test-`
...
it.instance("uses the temp directory", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("codemate-test-")
  }),
)
```
