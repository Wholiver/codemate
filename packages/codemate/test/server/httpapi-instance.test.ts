...
const directoryHeader = (dir: string) => HttpClientRequest.setHeader("x-codemate-directory", dir)
...
describe("instance HttpApi", () => {
  ...
  it.live("emits a sync fence header for fixed-workspace mutations", () =>
    Effect.gen(function* () {
      const originalWorkspaceID = Flag.codemate_WORKSPACE_ID
      Flag.codemate_WORKSPACE_ID = WorkspaceID.ascending()
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          Flag.codemate_WORKSPACE_ID = originalWorkspaceID
        }),
      )
        ...
        Effect.promise(() =>
          ExperimentalHttpApiServer.webHandler().handler(
            new Request(`http://localhost${path}`, {
              ...init,
              headers: { "x-codemate-directory": dir, "content-type": "application/json", ...init?.headers },
            }),
            handlerContext,
          ),
        )
