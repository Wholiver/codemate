...
function writeProviderAuthPlugin(dir: string) {
  return Effect.gen(function* () {
    ...
    yield* fs.makeDirectory(path.join(dir, ".codemate", "plugin"), { recursive: true })
    yield* fs.writeFileString(
      path.join(dir, ".codemate", "plugin", "provider-oauth-parity.ts"),
      [
        "export default {",
...
function writeFunctionOptionsPlugin(dir: string) {
  return Effect.gen(function* () {
    ...
    yield* fs.makeDirectory(path.join(dir, ".codemate", "plugin"), { recursive: true })
    yield* fs.writeFileString(
      path.join(dir, ".codemate", "plugin", "provider-function-options.ts"),
      [
        "export default {",
...
function writeProviderModelsMutationPlugin(dir: string) {
  return Effect.gen(function* () {
    ...
    yield* fs.makeDirectory(path.join(dir, ".codemate", "plugin"), { recursive: true })
    yield* fs.writeFileString(
      path.join(dir, ".codemate", "plugin", "provider-models-mutation.ts"),
      [
        "export default {",
...
function withProviderProject<A, E, R>(self: (dir: string) => Effect.Effect<A, E, R>) {
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem
    const path = yield* Path.Path
    const dir = yield* fs.makeTempDirectoryScoped({ prefix: "codemate-test-" })

    yield* fs.writeFileString(
      path.join(dir, "codemate.json"),
      JSON.stringify({ $schema: "https://codemate.ai/config.json", formatter: false, lsp: false }),
    )
...
describe("provider HttpApi", () => {
  it.live(
    ...
    withProviderProject((dir) =>
      Effect.gen(function* () {
        const headers = { "x-codemate-directory": dir, "content-type": "application/json" }
        const server = app()
  ...
  it.live("serves provider lists when auth loaders add runtime fetch options", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path
      const dir = yield* fs.makeTempDirectoryScoped({ prefix: "codemate-test-" })
      const previous = process.env.codemate_AUTH_CONTENT
      ...
      yield* Effect.sync(() => {
        process.env.codemate_AUTH_CONTENT = JSON.stringify({
          google: { type: "oauth", refresh: "dummy", access: "dummy", expires: 9999999999999 },
        })
      })
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          if (previous === undefined) delete process.env.codemate_AUTH_CONTENT
          if (previous !== undefined) process.env.codemate_AUTH_CONTENT = previous
        }),
      )
      const headers = { "x-codemate-directory": dir }
