 ...
 *
 * The script intentionally isolates `codemate_DB` before importing modules that touch
...
void (await import("@codemate-ai/core/util/log")).init({ print: false })
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "codemate.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "codemate.jsonc")).text(),
          )
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-codemate-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-codemate-directory")
  }),
  ...
  http.protected
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "codemate", model: "test" })}`,
      headers: ctx.headers(),
    }))
  ...
  http.protected
    ...
    .at((ctx) => ({
      ...
      headers: ctx.headers(),
      body: { agent: "orchestrator", model: { providerID: "test", modelID: "test-model" }, command: "printf shell-ok" },
    }))
    .json(
      200,
      (body) => {
