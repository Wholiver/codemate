...
describe("session HttpApi", () => {
  it.live(
    ...
    withTmp({ git: true, config: { formatter: false, lsp: false } }, (tmp) =>
      Effect.gen(function* () {
        const headers = { "x-codemate-directory": tmp.path }
    ...
    withTmp({ git: true, config: { formatter: false, lsp: false } }, (tmp) =>
      Effect.gen(function* () {
        ...
        const response = yield* request(pathFor(SessionPaths.get, { sessionID: session.id }), {
          headers: { "x-codemate-directory": tmp.path },
        })
  ...
  it.live(
    ...
    withTmp({ git: true, config: { formatter: false, lsp: false, share: "disabled" } }, (tmp) =>
      Effect.gen(function* () {
        const headers = { "x-codemate-directory": tmp.path, "content-type": "application/json" }
    ...
    withTmp({ git: true, config: { formatter: false, lsp: false, share: "disabled" } }, (tmp) =>
      Effect.gen(function* () {
        ...
        const created = yield* requestJson<Session.Info>(`${SessionPaths.create}?workspace=${workspace.id}`, {
          method: "POST",
          headers: { "x-codemate-directory": tmp.path, "content-type": "application/json" },
  ...
  it.live(
    ...
    withTmp({ git: true, config: { formatter: false, lsp: false } }, (tmp) =>
      Effect.gen(function* () {
        const currentDir = path.join(tmp.path, "packages", "codemate", "src")
