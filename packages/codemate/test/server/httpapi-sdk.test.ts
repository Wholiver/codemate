...
type Sdk = ReturnType<typeof createcodemateClient>
...
function app(serverPath: ServerPath, input?: { password?: string; username?: string }) {
  Flag.codemate_SERVER_PASSWORD = input?.password
  Flag.codemate_SERVER_USERNAME = input?.username
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            codemate_SERVER_PASSWORD: input?.password,
            codemate_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
...
function writeProjectSkill(dir: string) {
  return call(() =>
    Bun.write(
      path.join(dir, ".codemate", "skills", "project-rest-skill", "SKILL.md"),
      `---
      ...
      Effect.gen(function* () {
        ...
        const bad = yield* capture(() =>
          client("raw", directory, {
            password: "secret",
            headers: { authorization: authorization("codemate", "wrong") },
          }).file.read({ path: "hello.txt" }),
        )
        const good = yield* capture(() =>
          client("raw", directory, {
            password: "secret",
            headers: { authorization: authorization("codemate", "secret") },
          }).file.read({ path: "hello.txt" }),
        )
