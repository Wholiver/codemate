    ...
    Effect.gen(function* () {
      const handler = HttpRouter.toWebHandler(
        ExperimentalHttpApiServer.createRoutes().pipe(
          Layer.provide(ConfigProvider.layer(ConfigProvider.fromUnknown({ codemate_SERVER_PASSWORD: "secret" }))),
        ),
        { disableLogger: true },
      ).handler
      const response = yield* Effect.promise(() =>
        handler(
          new Request(new URL("/global/config", "http://localhost"), {
            headers: { origin: "https://app.codemate.ai" },
          }),
      ...
      expect(response.headers.get("access-control-allow-origin")).toBe("https://app.codemate.ai")
    }),
  )
