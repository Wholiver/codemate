...
const originalWorkspaces = Flag.codemate_EXPERIMENTAL_WORKSPACES
...
afterEach(async () => {
  Flag.codemate_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("session.list", () => {
  test("does not filter by directory when directory is omitted", async () => {
    ...
    await mkdir(path.join(tmp.path, "packages", "codemate"), { recursive: true })
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const current = await WithInstance.provide({
          directory: path.join(tmp.path, "packages", "codemate"),
  ...
  test("filters by directory when directory is provided", async () => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const ids = (await svc.list({ directory: path.join(tmp.path, "packages", "codemate") })).map((s) => s.id)
  ...
  test("filters by path and ignores directory when path is provided", async () => {
    Flag.codemate_EXPERIMENTAL_WORKSPACES = false
    await using tmp = await tmpdir({ git: true })
    await mkdir(path.join(tmp.path, "packages", "codemate", "src", "deep"), { recursive: true })
    await mkdir(path.join(tmp.path, "packages", "app"), { recursive: true })
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const current = await WithInstance.provide({
          directory: path.join(tmp.path, "packages", "codemate", "src"),
          fn: async () => svc.create({ title: "current" }),
        })
        const deeper = await WithInstance.provide({
          directory: path.join(tmp.path, "packages", "codemate", "src", "deep"),
  ...
  test("falls back to directory when filtering legacy sessions without path", async () => {
    Flag.codemate_EXPERIMENTAL_WORKSPACES = false
    await using tmp = await tmpdir({ git: true })
    await mkdir(path.join(tmp.path, "packages", "codemate", "src"), { recursive: true })
    await mkdir(path.join(tmp.path, "packages", "app"), { recursive: true })
