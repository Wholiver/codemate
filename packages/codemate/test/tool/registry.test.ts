...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".codemate")])),
})
...
afterEach(async () => {
  Flag.codemate_EXPERIMENTAL_SCOUT = originalExperimentalScout
...
describe("tool.registry", () => {
  it.instance("hides repo research tools unless experimental", () =>
    Effect.gen(function* () {
      Flag.codemate_EXPERIMENTAL_SCOUT = false
  ...
  it.instance("loads tools from .codemate/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const codemate = path.join(test.directory, ".codemate")
      const tool = path.join(codemate, "tool")
  ...
  it.instance("loads tools from .codemate/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const codemate = path.join(test.directory, ".codemate")
      const tools = path.join(codemate, "tools")
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(codemate, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@codemate-ai/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(codemate, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(codemate, "node_modules", "cowsay")
