...
describe("tool.read external_directory permission", () => {
  ...
  it.live("does not ask for external_directory permission when reading configured references", () =>
    experimentalScout(
      Effect.gen(function* () {
        const fs = yield* AppFileSystem.Service
        const cache = path.join(Global.Path.repos, "github.com", "codemate-read-reference", "repo")
        ...
        const remoteDir = path.join(remoteRoot, "codemate-read-reference")
        ...
        const dir = yield* tmpdirScoped({
          ...
          config: {
            reference: {
              docs: "codemate-read-reference/repo",
            },
          },
        })
...
describe("tool.read env file permissions", () => {
  ...
  for (const agentName of ["orchestrator"] as const) {
