            ...
            get: () =>
              Effect.succeed({
                ...
                plugin_origins: plugins.map((plugin) => ({
                  spec: plugin,
                  source: path.join(directory, "codemate.json"),
...
describe("plugin.auth-override", () => {
  test("user plugin overrides built-in github-copilot auth", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const pluginDir = path.join(dir, ".codemate", "plugin")
    ...
    const plugin = pathToFileURL(path.join(tmp.path, ".codemate", "plugin", "custom-copilot-auth.ts")).href
