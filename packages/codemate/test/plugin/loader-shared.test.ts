...
const disableDefault = process.env.codemate_DISABLE_DEFAULT_PLUGINS
process.env.codemate_DISABLE_DEFAULT_PLUGINS = "1"

const { Plugin } = await import("../../src/plugin/index")
const { PluginLoader } = await import("../../src/plugin/loader")
const { readPackageThemes } = await import("../../src/plugin/shared")
const { Bus } = await import("../../src/bus")
const { Npm } = await import("@codemate-ai/core/npm")
const { TestConfig } = await import("../fixture/config")

afterAll(() => {
  if (disableDefault === undefined) {
    delete process.env.codemate_DISABLE_DEFAULT_PLUGINS
    return
  }
  process.env.codemate_DISABLE_DEFAULT_PLUGINS = disableDefault
})
...
async function load(dir: string) {
  const source = path.join(dir, "codemate.json")
...
describe("plugin.loader.shared", () => {
  ...
  test("loads npm server plugin from package ./server export", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "codemate.json"), JSON.stringify({ plugin: ["acme-plugin@1.0.0"] }, null, 2))
  ...
  test("rejects npm server export that resolves outside plugin directory", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "codemate.json"), JSON.stringify({ plugin: ["acme-plugin"] }, null, 2))
  ...
  test("skips legacy codex and copilot auth plugin specs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(
          ...
          JSON.stringify(
            {
              plugin: ["codemate-openai-codex-auth@1.0.0", "codemate-copilot-auth@1.0.0", "regular-plugin@1.0.0"],
            },
            null,
            2,
          ),
    ...
    try {
      ...
      expect(pkgs).not.toContain("codemate-openai-codex-auth@1.0.0")
      expect(pkgs).not.toContain("codemate-copilot-auth@1.0.0")
    } finally {
      install.mockRestore()
    }
  })
  ...
  test("continues loading plugins when plugin init throws", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "codemate.json"), JSON.stringify({ plugin: [file, ok] }, null, 2))
  ...
  test("continues loading plugins when plugin import fails", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "codemate.json"), JSON.stringify({ plugin: [missing, ok] }, null, 2))
...
`,
        ...
        await Bun.write(path.join(dir, "codemate.json"), JSON.stringify({ plugin: [aSpec, bSpec] }, null, 2))
  ...
  test("skips external plugins in pure mode", async () => {
    ...
    const pure = process.env.codemate_PURE
    ...
    try {
      ...
      if (pure === undefined) {
        delete process.env.codemate_PURE
      } else {
        process.env.codemate_PURE = pure
      }
    }
  })
