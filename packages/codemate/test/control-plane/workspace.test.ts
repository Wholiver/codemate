...
const originalWorkspacesFlag = Flag.codemate_EXPERIMENTAL_WORKSPACES
const originalEnv = {
  codemate_AUTH_CONTENT: process.env.codemate_AUTH_CONTENT,
...
beforeEach(() => {
  Database.close()
  Flag.codemate_EXPERIMENTAL_WORKSPACES = true
  restoreEnv()
})
...
afterEach(async () => {
  ...
  Flag.codemate_EXPERIMENTAL_WORKSPACES = originalWorkspacesFlag
  restoreEnv()
  await resetDatabase()
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@codemate.test"`.cwd(dir).quiet()
...
describe("workspace CRUD", () => {
  ...
  test("create configures, persists, creates, starts local sync, and passes environment", async () => {
    await withInstance(async (dir) => {
      process.env.codemate_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
      process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
      process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
      process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=codemate-test"
      ...
      expect(JSON.parse(recorded.calls.create[0].env.codemate_AUTH_CONTENT ?? "{}")).toEqual({
        test: { type: "api", key: "secret" },
      })
      expect(recorded.calls.create[0].env.codemate_WORKSPACE_ID).toBe(workspaceID)
      expect(recorded.calls.create[0].env.codemate_EXPERIMENTAL_WORKSPACES).toBe("true")
      ...
      expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=codemate-test")
...
describe("workspace sync state", () => {
  test("startWorkspaceSyncing is disabled by the experimental workspace flag", async () => {
    await withInstance(async (dir) => {
      Flag.codemate_EXPERIMENTAL_WORKSPACES = false
