...
import type { NamedError } from "@codemate-ai/core/util/error"
...
describe("session.retry.retryable", () => {
  ...
  test("does not map free usage errors to provider-specific upsell actions", () => {
    const error = MessageV2.APIError.Schema.parse(
      new MessageV2.APIError({
    ...
    expect(SessionRetry.retryable(error, "codemate")).toEqual({ message: "Free usage exceeded" })
  })

  test("does not map Go subscription limits to workspace upsell actions", () => {
    const error = MessageV2.APIError.Schema.parse(
      new MessageV2.APIError({
    ...
    expect(SessionRetry.retryable(error, "codemate-go")).toEqual({
      message: "Subscription quota exceeded. You can continue using free models.",
    })
  })
})
