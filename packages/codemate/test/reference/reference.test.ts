...
const experimentalScout = <A, E, R>(self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = Flag.codemate_EXPERIMENTAL_SCOUT
      Flag.codemate_EXPERIMENTAL_SCOUT = true
      return previous
    }),
    () => self,
    (previous) =>
      Effect.sync(() => {
        Flag.codemate_EXPERIMENTAL_SCOUT = previous
      }),
  )
...
const githubBase = <A, E, R>(url: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.codemate_REPO_CLONE_GITHUB_BASE_URL
      process.env.codemate_REPO_CLONE_GITHUB_BASE_URL = url
      return previous
    }),
    () => self,
    (previous) =>
      Effect.sync(() => {
        if (previous) process.env.codemate_REPO_CLONE_GITHUB_BASE_URL = previous
        else delete process.env.codemate_REPO_CLONE_GITHUB_BASE_URL
      }),
  )
...
describe("reference", () => {
  it.live("resolves local and git references", () =>
    Effect.gen(function* () {
      const root = path.resolve("codemate-reference-root")
      ...
      provideTmpdirInstance(
        (_dir) =>
          Effect.gen(function* () {
            ...
            const cache = path.join(Global.Path.repos, "github.com", "codemate-reference-test", "repo")
            ...
            const remoteDir = path.join(remoteRoot, "codemate-reference-test")
          ...
          }),
        {
          config: {
            reference: {
              docs: "codemate-reference-test/repo",
            },
          },
        },
      ),
  ...
  it.live("refreshes configured git references on new instance init", () =>
    experimentalScout(
      Effect.gen(function* () {
        const fs = yield* AppFileSystem.Service
        const cache = path.join(Global.Path.repos, "github.com", "codemate-reference-refresh", "repo")
        yield* fs.remove(cache, { recursive: true }).pipe(Effect.ignore)
        yield* Effect.addFinalizer(() => fs.remove(cache, { recursive: true }).pipe(Effect.ignore))

        const source = yield* tmpdirScoped({ git: true })
        const remoteRoot = yield* tmpdirScoped()
        const remoteDir = path.join(remoteRoot, "codemate-reference-refresh")
        const remoteRepo = path.join(remoteDir, "repo.git")
        ...
        yield* githubBase(
          ...
          provideTmpdirInstance(
              ...
              }),
            {
              config: {
                reference: {
                  docs: "codemate-reference-refresh/repo",
                },
              },
            },
          ),
