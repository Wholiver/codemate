...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "codemate.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "codemate.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "codemate.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "codemate.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["codemate.json", "codemate.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "codemate.json"),
        path.join(tmp.path, "codemate.jsonc"),
        path.join(project, "codemate.json"),
        path.join(project, "codemate.jsonc"),
      ])
    })
  })
