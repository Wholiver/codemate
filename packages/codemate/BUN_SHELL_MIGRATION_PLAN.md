...
Replace runtime Bun shell template-tag usage in `packages/codemate/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/codemate/src` is removed except:
