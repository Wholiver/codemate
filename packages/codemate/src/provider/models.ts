...
import { Flock } from "@codemate-ai/core/util/flock"
...
export class Service extends Context.Service<Service, Interface>()("@codemate/ModelsDev") {}
...
export const layer: Layer.Layer<Service, never, AppFileSystem.Service | HttpClient.HttpClient> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const source = Flag.codemate_MODELS_URL || "https://models.dev"
    ...
    const loadFromDisk = fs.readJson(Flag.codemate_MODELS_PATH ?? filepath).pipe(
    ...
    const populate = Effect.gen(function* () {
      ...
      if (Flag.codemate_DISABLE_MODELS_FETCH) return {}
      // Flock is cross-process: concurrent codemate CLIs can race on this cache file.
    ...
    if (!Flag.codemate_DISABLE_MODELS_FETCH && !process.argv.includes("--get-yargs-completions")) {
