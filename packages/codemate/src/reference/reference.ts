...
export class Service extends Context.Service<Service, Interface>()("@codemate/Reference") {}
  ...
  Effect.gen(function* () {
    ...
    return Service.of({
      init: Effect.fn("Reference.init")(function* () {
        if (!Flag.codemate_EXPERIMENTAL_SCOUT) return
      ...
      contains: Effect.fn("Reference.contains")(function* (target?: string) {
        if (!Flag.codemate_EXPERIMENTAL_SCOUT) return false
        const full = normalizedTarget(target)
