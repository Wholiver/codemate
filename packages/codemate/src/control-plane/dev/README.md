This is a plugin to simulate a remote environment locally. Add this to `.codemate/codemate.jsonc`:

```json
  "plugin": ["../packages/codemate/src/control-plane/dev/debug-workspace-plugin.ts"],
```

In a separate terminal, run a separate Codemate server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/codemate/script/run-workspace-server
```

With the plugin install, you can now run Codemate and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
