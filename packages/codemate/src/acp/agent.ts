...
import type { AssistantMessage, Event, CodemateClient, SessionMessageResponse, ToolPart } from "@codemate-ai/sdk/v2"
...
export async function init({ sdk: _sdk }: { sdk: CodemateClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `codemate auth login` in the terminal",
      name: "Login with codemate",
      id: "codemate-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // Codemate stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const codemateProvider = providers.find((p) => p.id === "codemate")
  if (codemateProvider) {
    if (codemateProvider.models["big-pickle"]) {
      return { providerID: ProviderID.codemate, modelID: ModelID.make("big-pickle") }
    }
    const [best] = Provider.sort(Object.values(codemateProvider.models))
    if (best) {
      return {
