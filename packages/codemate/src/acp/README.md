...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for codemate.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal codemate sessions
...
codemate acp --cwd /path/to/project
```
...
```bash
CODEMATE_ENABLE_QUESTION_TOOL=1 codemate acp
```
...
- **Terminal Integration**: Full terminal support via codemate's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | codemate acp
```
...
ACP sessions map cleanly to codemate's internal session model:
