...
import { Module } from "@codemate-ai/core/util/module"
...
export const Vue: Info = {
  ...
  async spawn(root) {
    ...
    const args: string[] = []
    if (!binary) {
      if (Flag.CODEMATE_DISABLE_LSP_DOWNLOAD) return
...
export const JDTLS: Info = {
  ...
  async spawn(root) {
    ...
    const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "codemate-jdtls-data"))
    return {
      process: spawn(
        java,
        [
