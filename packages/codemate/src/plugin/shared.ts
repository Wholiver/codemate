...
export const DEPRECATED_PLUGIN_PACKAGES = ["codemate-openai-codex-auth", "codemate-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, codemateVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(codemateVersion) || semver.major(codemateVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.codemate
  if (typeof range !== "string") return
  if (!semver.satisfies(codemateVersion, range)) {
    throw new Error(`Plugin requires codemate ${range} but running ${codemateVersion}`)
  }
}
