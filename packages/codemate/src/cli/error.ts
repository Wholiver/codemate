...
export function FormatError(input: unknown) {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    return `MCP server "${(input as ErrorLike).data?.name}" failed. Note, codemate does not support MCP authentication yet.`
  }
  ...
  if (NamedError.hasName(input, "ProviderModelNotFoundError")) {
    ...
    return [
      ...
      `Try: \`codemate models\` to list available models`,
      `Or check your config (codemate.json) provider/model names`,
    ].join("\n")
  }
