...
const TUI_SCHEMA_URL = "https://codemate.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from codemate.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const codemate = await codemateFiles(input)
  for (const file of codemate) {
...
async function codemateFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "codemate"),
    ...(await Filesystem.findUp(["codemate.json", "codemate.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
