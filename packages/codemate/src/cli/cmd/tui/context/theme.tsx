...
import codemate from "./theme/codemate.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@codemate-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "codemate")
        draft.active = typeof active === "string" ? active : "codemate"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.codemate, store.mode)
    })
