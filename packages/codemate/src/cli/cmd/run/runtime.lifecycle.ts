...
function footerLabels(input: Pick<RunInput, "agent" | "model" | "variant">): FooterLabels {
  const agentLabel = Locale.titlecase(input.agent ?? "orchestrator")
...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "codemate.agent.name": input.agent,
      "codemate.directory": input.directory,
      "codemate.first": input.first,
      "codemate.model.provider": input.model?.providerID,
      "codemate.model.id": input.model?.modelID,
      "codemate.model.variant": input.variant,
      ...
      try {
        ...
        const close = async (next: {
          ...
          return withRunSpan(
            "RunLifecycle.close",
            {
              "codemate.show_exit": next.showExit,
