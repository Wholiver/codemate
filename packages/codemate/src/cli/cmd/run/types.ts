...
import type { codemateClient, PermissionRequest, QuestionRequest, ToolPart } from "@codemate-ai/sdk/v2"
...
type PromptModel = Parameters<codemateClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<codemateClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<codemateClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<codemateClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<codemateClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<codemateClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<codemateClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<codemateClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<codemateClient["question"]["reject"]>[0]
