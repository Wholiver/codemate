...
import { createcodemateClient } from "@codemate-ai/sdk/v2"
...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "codemate.mode": input.resolveSession ? "local" : "attach",
      "codemate.initial_input": !!input.initialInput,
      "codemate.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "codemate.directory": ctx.directory,
        "codemate.resume": ctx.resume === true,
        "codemate.agent.name": state.agent,
        "codemate.model.provider": state.model?.providerID,
        "codemate.model.id": state.model?.modelID,
        "codemate.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "codemate.model.provider": model.providerID,
            "codemate.model.id": model.modelID,
            "codemate.model.variant": state.activeVariant,
          })
          return {
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "codemate.model.id": state.model?.modelID,
                "codemate.model.variant": state.activeVariant,
                "codemate.prompt.chars": prompt.text.length,
                "codemate.prompt.parts": prompt.parts.length,
                "codemate.prompt.include_files": includeFiles,
                "codemate.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "codemate.directory": input.directory,
      "codemate.initial_input": !!input.initialInput,
      "codemate.demo": input.demo,
    },
    async () => {
