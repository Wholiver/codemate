...
export const ProvidersListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.list")(function* (_args) {
    ...
    const results = Object.entries(yield* Effect.orDie(authSvc.all())).filter(
      ([providerID]) => !ProviderUnsupported.isUnsupportedProviderID(providerID),
    )
    ...
    for (const [providerID, provider] of Object.entries(database)) {
      if (ProviderUnsupported.isUnsupportedProviderID(providerID)) continue
...
export const ProvidersLoginCommand = effectCmd({
  ...
  builder: (yargs) =>
    yargs
      .positional("url", {
        describe: "codemate auth provider",
        type: "string",
      })
      .option("provider", {
        alias: ["p"],
  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    if (args.url) {
      ...
      const wellknown = (yield* cliTry(`Failed to load auth provider metadata from ${url}: `, () =>
        fetch(`${url}/.well-known/codemate`).then((x) => x.json()),
      )) as {
    ...
    if (provider === "other") {
      ...
      if (ProviderUnsupported.isUnsupportedProviderID(provider)) {
        return yield* fail(`Provider is not supported: ${provider}`)
      }
      ...
      yield* Prompt.log.warn(
        `This only stores a credential for ${provider} - you will need configure it in codemate.json, check the docs for examples.`,
      )
    }
    ...
    if (provider === "amazon-bedrock") {
      yield* Prompt.log.info(
        "Amazon Bedrock authentication priority:\n" +
          ...
          "Configure via codemate.json options (profile, region, endpoint) or\n" +
    ...
    if (["cloudflare", "cloudflare-ai-gateway"].includes(provider)) {
      yield* Prompt.log.info(
        "Cloudflare AI Gateway can be configured with CLOUDFLARE_GATEWAY_ID, CLOUDFLARE_ACCOUNT_ID, and CLOUDFLARE_API_TOKEN environment variables. Read more: https://codemate.ai/docs/providers/#cloudflare-ai-gateway",
      )
    }
