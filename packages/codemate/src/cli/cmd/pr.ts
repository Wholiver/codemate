...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run codemate",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for codemate session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found codemate session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["codemate", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting codemate...")
        UI.println()

        const codemateArgs = sessionId ? ["-s", sessionId] : []
        const codemateProcess = Process.spawn(["codemate", ...codemateArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await codemateProcess.exited
        if (code !== 0) throw new Error(`codemate exited with code ${code}`)
      },
    })
  },
})
