...
export const PrCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
      ...
      if (prInfo?.body) {
        ...
        if (sessionMatch) {
          ...
          const importResult = yield* Effect.promise(() =>
            Process.text(["codemate", "import", sessionUrl], { nothrow: true }),
          )
    ...
    const code = yield* Effect.promise(
      () =>
        Process.spawn(["codemate", ...codemateArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
    ...
    if (code !== 0) return yield* Effect.die(new Error(`codemate exited with code ${code}`))
  }),
})
