// CLI entry point for `codemate run`.
//
// Handles three modes:
...
//      codemate server and runs interactive mode against it.
//
...
import { createcodemateClient, type codemateClient, type ToolPart } from "@codemate-ai/sdk/v2"
import { Agent } from "@/agent/agent"
import { Permission } from "@/permission"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

const runtimeTask = import("./run/runtime")
type ModelInput = Parameters<codemateClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run codemate with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running codemate server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to codemate_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to codemate_SERVER_USERNAME or 'codemate')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: codemateClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: codemateClient, sessionID: string) {
        const cfg = await sdk.config.get()
        if (!cfg.data) return
        if (cfg.data.share !== "auto" && !Flag.codemate_AUTO_SHARE && !args.share) return
      ...
      async function current(sdk: codemateClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: codemateClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: codemateClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: codemateClient) {
        ...
        async function loop(client: codemateClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createcodemateClient({
        baseUrl: "http://codemate.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
