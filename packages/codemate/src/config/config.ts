...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for codemate serve and web commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://codemate.ai/docs/agents" }),
...
export class Service extends Context.Service<Service, Interface>()("@codemate/Config") {}

function globalConfigFile() {
  const candidates = ["codemate.jsonc", "codemate.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://codemate.ai/config.json"
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            log.debug("fetching remote config", { url: `${url}/.well-known/codemate` })
            const response = yield* Effect.promise(() => fetch(`${url}/.well-known/codemate`))
            if (!response.ok) {
        ...
        for (const dir of directories) {
          ...
          // Auto-discovered plugins under `.codemate/plugin(s)` are already local files, so ConfigPlugin.load
