...
Practical reference for migrating data types in `packages/codemate` from
...
schema via `@codemate-ai/core/effect-zod`.
...
The walker in `@codemate-ai/core/effect-zod` exposes two explicit escape hatches for
...
All of `packages/codemate/src/config/` has been migrated. Files that still
...
- Use `@codemate-ai/core/effect-zod` for all Schema → Zod conversion.
