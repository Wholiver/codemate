...
Practical reference for new and migrated Effect code in `packages/codemate`.
...
When a service uses Effect Schema internally but needs Zod schemas for the HTTP layer, derive Zod from Schema using the `zod()` helper from `@codemate-ai/core/effect-zod`:
