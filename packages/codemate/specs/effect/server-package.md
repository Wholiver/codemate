...
Practical reference for extracting a future `packages/server` from the current `packages/codemate` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all codemate services, Effect-first source of truth
- `packages/server` - codemate server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full codemate server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/codemate`.
...
- those slices are mounted into the Hono server behind `codemate_EXPERIMENTAL_HTTPAPI`
...
The intended direction here is the same, but the current `codemate` package split is earlier in the migration.
...
- the current server host still lives in `packages/codemate`
...
- if `packages/server` imports services from `packages/codemate` while `packages/codemate` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/codemate` as the temporary Hono host
...
- `packages/server` must not import from `packages/codemate`

Allowed in phase 1:

- `packages/codemate` imports `packages/server`
...
- `packages/codemate` shrinks or disappears as package responsibilities are fully split
...
- `packages/codemate` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/codemate`
...
Keep in `packages/codemate` for now:
...
- host mounting from `packages/codemate`
...
- no imports from `packages/codemate` inside `packages/server`
...
- `packages/server` must still not import from `packages/codemate`
...
### PR 4. Mount `packages/server` question from `packages/codemate`

Scope:

- replace local experimental question route wiring in `packages/codemate`
...
- mount it in parallel from `packages/codemate`
...
- move adapter selection and server startup out of `packages/codemate`

Rules:

- do not start this while `packages/server` still depends on `packages/codemate`
...
- host logic no longer lives in `packages/codemate`
...
3. the route is mounted from the current host in `packages/codemate`
...
- `packages/server` imports services or runtime from `packages/codemate`
- `packages/codemate` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/codemate` imports
...
- [ ] mount `question` from `packages/codemate`
...
2. keep `packages/codemate` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/codemate`, stop and restructure the boundary first.
