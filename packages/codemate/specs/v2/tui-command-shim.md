...
Delete `packages/codemate/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/codemate/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/codemate/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/codemate`
- TUI plugin loader tests in `packages/codemate` if runtime plugin API wiring changed
