...
- Author package entrypoint is `@codemate-ai/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/codemate-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@codemate-ai/plugin/tui`.
- `@codemate-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
- The value is a semver range checked against the running codemate version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.codemate` is absent, no check is performed (backward compatible).
...
- `codemate plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `codemate plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.codemate` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.codemate`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.codemate`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.codemate` dir or `codemate_CONFIG_DIR`, codemate installs `@codemate-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@codemate-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus codemate's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@codemate-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.codemate/themes` area near the plugin config source.
...
- `--pure` / `codemate_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.codemate/plugins/tui-smoke.tsx`
- Local vim plugin: `.codemate/plugins/tui-vim.tsx`
- Local smoke config: `.codemate/tui.json`
- Local smoke theme: `.codemate/plugins/smoke-theme.json`
