...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-codemate-directory", "directory"],
    ["x-codemate-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-codemate-directory")
  next.headers.delete("x-codemate-workspace")
  return next
}
...
export function createcodemateClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  ...
  if (config?.directory) {
    config.headers = {
      ...config.headers,
      "x-codemate-directory": encodeURIComponent(config.directory),
    }
  }
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-codemate-workspace": config.experimental_workspaceID,
    }
  }
