...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("codemate-find")
  api.delete("codemate-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("codemate-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("codemate-find", new Highlight(...rest))
    return true
  }
