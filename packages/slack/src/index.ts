...
import { createcodemate, type ToolPart } from "@codemate-ai/sdk"
...
console.log("🚀 Starting codemate server...")
const codemate = await createcodemate({
  port: 0,
})
console.log("✅ codemate server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await codemate.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new codemate session...")
    const { client, server } = codemate
    ...
    console.log("✅ Created codemate session:", createResult.data.id)
  ...
  console.log("📝 Sending to codemate:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 codemate response:", JSON.stringify(result, null, 2))
