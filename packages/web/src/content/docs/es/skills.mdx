...
Las habilidades del agente permiten a codemate descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
codemate busca estas ubicaciones:

- Configuración del proyecto: `.codemate/skills/<name>/SKILL.md`
- Configuración global: `~/.config/codemate/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, codemate sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.codemate/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/codemate/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.codemate/skills/git-release/SKILL.md` así:
...
codemate enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `codemate.json`:
...
**Para agentes integrados** (en `codemate.json`):
