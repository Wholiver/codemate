---
title: Servidor
description: Interactuar con el servidor codemate a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `codemate serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente codemate puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `codemate.local` |
...
Configure `CODEMATE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `codemate`, o configure `CODEMATE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `codemate serve` como a `codemate web`.
...
Cuando ejecuta `codemate`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor codemate para interactuar con codemate mediante programación.
:::

Esta arquitectura permite que codemate admita múltiples clientes y le permite interactuar con codemate mediante programación.

Puede ejecutar `codemate serve` para iniciar un servidor independiente. Si tienes el
codemate TUI ejecutándose, `codemate serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Codemate [IDE](/docs/ide).
...
El servidor codemate expone las siguientes API.
