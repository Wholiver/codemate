---
title: Complementos
description: Escriba sus propios complementos para extender codemate.
---

Los complementos le permiten extender codemate al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de codemate.
...
- `.codemate/plugins/` - Complementos a nivel de proyecto
- `~/.config/codemate/plugins/` - Complementos globales
...
```json title="codemate.json"
{
  ...
  "plugin": ["codemate-helicone-session", "codemate-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/codemate/node_modules/`.
...
1. Configuración global (`~/.config/codemate/codemate.json`)
2. Configuración del proyecto (`codemate.json`)
3. Directorio global de complementos (`~/.config/codemate/plugins/`)
4. Directorio de complementos del proyecto (`.codemate/plugins/`)
...
```json title=".codemate/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
codemate ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".codemate/plugins/my-plugin.ts"
...
```js title=".codemate/plugins/example.js"
...
- `client`: Un cliente SDK codemate para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar codemate.
...
```js title=".codemate/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "codemate"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio codemate, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que codemate lea archivos `.env`:

```javascript title=".codemate/plugins/env-protection.js"
...
```javascript title=".codemate/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a codemate:

```ts title=".codemate/plugins/custom-tools.ts"
import { type Plugin, tool } from "@codemate-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que codemate puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para codemate junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://codemate.ai/docs/sdk) para obtener más detalles.
...
```ts title=".codemate/plugins/compaction.ts"
import type { Plugin } from "@codemate-ai/plugin"
...
```ts title=".codemate/plugins/custom-compaction.ts"
import type { Plugin } from "@codemate-ai/plugin"
