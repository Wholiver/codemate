...
description: Cree herramientas que LLM pueda llamar en codemate.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de codemate como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.codemate/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/codemate/tools/`.
...
```ts title=".codemate/tools/database.ts" {1}
...
```ts title=".codemate/tools/math.ts"
import { tool } from "@codemate-ai/plugin"
...
```ts title=".codemate/tools/bash.ts"
import { tool } from "@codemate-ai/plugin"
...
```ts title=".codemate/tools/project.ts" {8}
import { tool } from "@codemate-ai/plugin"
...
```python title=".codemate/tools/add.py"
import sys
...
```ts title=".codemate/tools/python-add.ts" {10}
import { tool } from "@codemate-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".codemate/tools/add.py")
