...
Con codemate puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, codemate usa nuestro propio tema `codemate`.
...
codemate viene con varios temas integrados.
...
- Quiere que codemate coincida con la apariencia de su terminal
...
codemate admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/codemate/themes/*.json` o `$XDG_CONFIG_HOME/codemate/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.codemate/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.codemate/themes/*.json`
