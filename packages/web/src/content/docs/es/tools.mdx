...
Las herramientas permiten que LLM realice acciones en su código base. Codemate viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Codemate.
...
:::note
Esta herramienta solo está disponible cuando `CODEMATE_EXPERIMENTAL_LSP_TOOL=true` (o `CODEMATE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Codemate o cuando la variable de entorno `CODEMATE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Codemate:

```bash
CODEMATE_ENABLE_EXA=1 codemate
```
