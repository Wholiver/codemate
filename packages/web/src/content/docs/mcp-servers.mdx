...
You can add external tools to codemate using the _Model Context Protocol_, or MCP. codemate supports both local and remote servers.
...
You can define MCP servers in your [codemate Config](https://codemate.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/codemate` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
codemate automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, codemate will:
...
If the server requires authentication, codemate will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `codemate mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, codemate will store the tokens securely in `~/.local/share/codemate/mcp-auth.json`.
...
Your MCPs are available as tools in codemate, alongside built-in tools. So you can manage them through the codemate config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect codemate to your Sentry account.
