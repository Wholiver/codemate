...
Создайте `.codemate/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию codemate или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем codemate [config](/docs/config):
...
- Глобальный: `~/.config/codemate/commands/`
- Для каждого проекта: `.codemate/commands/`
...
codemate включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
