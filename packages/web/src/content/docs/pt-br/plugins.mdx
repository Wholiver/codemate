---
title: Plugins
description: Escreva seus próprios plugins para estender o codemate.
---

Plugins permitem que você estenda o codemate conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do codemate.
...
- `.codemate/plugins/` - Plugins em nível de projeto
- `~/.config/codemate/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/codemate/node_modules/`.
...
1. Configuração global (`~/.config/codemate/codemate.json`)
2. Configuração do projeto (`codemate.json`)
3. Diretório de plugins global (`~/.config/codemate/plugins/`)
4. Diretório de plugins do projeto (`.codemate/plugins/`)
...
O codemate executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do codemate para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o codemate.
...
:::note
Se você estiver usando o aplicativo desktop codemate, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o codemate de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao codemate:
...
O helper `tool` cria uma ferramenta personalizada que o codemate pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o codemate junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://codemate.ai/docs/sdk) para detalhes.
