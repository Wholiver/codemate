...
O Codemate Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

:::note
O Codemate Go está atualmente em beta.
:::

O Go funciona como qualquer outro provedor no Codemate. Você assina o Codemate Go e
...
O Codemate Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O Codemate Go funciona como qualquer outro provedor no Codemate.

1. Você entra no **<a href={console}>Codemate Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Codemate Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Codemate Go.
:::
...
O Codemate Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do Codemate
usa o formato `codemate-go/<model-id>`. Por exemplo, para o Kimi K2.6, você usaria
`codemate-go/kimi-k2.6` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Codemate também.
