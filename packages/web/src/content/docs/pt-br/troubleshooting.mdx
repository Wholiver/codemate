...
Para depurar problemas com o codemate, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\codemate\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `codemate --log-level DEBUG`.

---

## Armazenamento

O codemate armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/codemate/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\codemate`
...
O codemate Desktop executa um servidor codemate local (o sidecar `codemate-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `Codemate` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/codemate/codemate.jsonc` (ou `~/.config/codemate/codemate.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/codemate/codemate.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\codemate\codemate.jsonc`
...
O codemate também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/codemate/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\codemate\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o codemate possa reconstruí-lo.

1. Saia completamente do codemate Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/codemate`
- **Linux**: exclua `~/.cache/codemate` (ou execute `rm -rf ~/.cache/codemate`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\codemate`

3. Reinicie o codemate Desktop.

---

### Corrigindo problemas de conexão com o servidor

O codemate Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `codemate.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `CODEMATE_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `CODEMATE_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o codemate Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do codemate.

---

### Notificações não aparecendo

O codemate Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o codemate nas configurações do seu sistema operacional, e
...
1. Saia do codemate Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo codemate Desktop):

- `codemate.settings.dat` (URL do servidor padrão do desktop)
- `codemate.global.dat` e `codemate.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o codemate:
...
Para descobrir quais modelos você tem acesso, execute `codemate models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\codemate`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O codemate instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\codemate`

2. Reinicie o codemate para reinstalar os pacotes de provedores mais recentes

Isso forçará o codemate a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O codemate detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
