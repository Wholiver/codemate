...
As habilidades do agente permitem que o codemate descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O codemate pesquisa nesses locais:

- Configuração do projeto: `.codemate/skills/<name>/SKILL.md`
- Configuração global: `~/.config/codemate/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o codemate sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.codemate/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/codemate/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.codemate/skills/git-release/SKILL.md` assim:
...
O codemate lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `codemate.json`:
...
**Para agentes embutidos** (em `codemate.json`):
