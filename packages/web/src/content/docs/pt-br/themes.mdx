...
Com o codemate, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o codemate usa nosso próprio tema `codemate`.
...
O codemate vem com vários temas embutidos.
...
- Querem que o codemate corresponda à aparência do seu terminal
...
O codemate suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/codemate/themes/*.json` ou `$XDG_CONFIG_HOME/codemate/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.codemate/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.codemate/themes/*.json`
