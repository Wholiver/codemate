---
title: Servidor
description: Interaja com o servidor codemate via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `codemate serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente codemate pode usar.
...
```bash
codemate serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `codemate.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
codemate serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `codemate_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `codemate`, ou defina `codemate_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `codemate serve` quanto ao `codemate web`.

```bash
codemate_SERVER_PASSWORD=your-password codemate serve
```
...
Quando você executa `codemate`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor codemate para interagir com o codemate programaticamente.
:::

Essa arquitetura permite que o codemate suporte múltiplos clientes e permite que você interaja com o codemate programaticamente.

Você pode executar `codemate serve` para iniciar um servidor autônomo. Se você tiver o TUI do codemate em execução, `codemate serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do codemate [IDE](/docs/ide).
...
O servidor codemate expõe as seguintes APIs.
