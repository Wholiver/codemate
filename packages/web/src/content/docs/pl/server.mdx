---
title: Serwer
description: Interakcja z serwerem codemate za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `codemate serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient codemate.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `codemate.local` |
...
Ustaw `codemate_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `codemate` lub ustaw `codemate_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `codemate serve`, jak i `codemate web`.
...
Kiedy uruchomisz `codemate`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera codemate do programowej interakcji z codemate.
:::

Ta architektura umożliwia obsługę wielu klientów przez codemate i programową interakcję z kodem otwartym.

Możesz uruchomić `codemate serve`, aby uruchomić samodzielny serwer. Jeśli masz
codemate TUI działa, `codemate serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki codemate [IDE](/docs/ide).
...
Serwer codemate udostępnia następujące interfejsy API.
