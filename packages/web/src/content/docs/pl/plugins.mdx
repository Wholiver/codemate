---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć codemate.
---

Wtyczki umożliwiające wyłączenie codemate poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania codemate.
...
- `.codemate/plugins/` - Wtyczki na poziomie projektu
- `~/.config/codemate/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/codemate/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/codemate/codemate.json`)
2. Konfiguracja projektu (`codemate.json`)
3. Globalny katalog wtyczek (`~/.config/codemate/plugins/`)
4. Katalog wtyczek projektu (`.codemate/plugins/`)
...
codemate uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z codemate do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do codemate.
...
:::note
Jeśli korzystasz z aplikacji komputerowej codemate, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj codemate czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do codemate:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod codemate. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://codemate.ai/docs/sdk).
