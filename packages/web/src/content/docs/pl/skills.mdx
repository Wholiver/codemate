...
Umiejętności agenta pozwalają codemate odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
codemate przeszukuje te lokalizacje:

- Project config: `.codemate/skills/<name>/SKILL.md`
- Global config: `~/.config/codemate/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu codemate przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.codemate/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/codemate/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.codemate/skills/git-release/SKILL.md` w ten sposób:
...
codemate wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `codemate.json`:
...
**Dla agentów wbudowanych** (w `codemate.json`):
