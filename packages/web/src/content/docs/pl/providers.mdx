---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w codemate.
---

import config from "../../../../config.mjs"
export const console = config.console

codemate używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji codemate.
...
w `~/.local/share/codemate/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim codemate
config.
...
codemate Zen to lista modeli dostarczonych przez zespół codemate, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z codemate. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od codemate Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `codemate Zen` i przejdź do [codemate.ai/auth](https://codemate.ai/zen).
...
Działa jak każdy inny dostawca w codemate i jest całkowicie opcjonalny w użyciu.

---

## codemate Go

codemate Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół codemate, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z codemate.

1. Uruchom polecenie `/connect` w TUI, wybierz `codemate Go` i przejdź do [codemate.ai/auth](https://codemate.ai/zen).
...
Aby używać Amazon Bedrock z codemate:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania codemate:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `codemate.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w codemate nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować codemate do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod codemate działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji codemate.
...
codemate używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować codemate
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`codemate.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [codemate-gitlab-auth](https://www.npmjs.com/package/codemate-gitlab-auth).

##### Konfiguracja

Customize through `codemate.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem codemate:
...
Aby używać Google Vertex AI z codemate:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania codemate.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą codemate, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [codemate-helicone-session](https://github.com/H2Shami/codemate-helicone-session), aby automatycznie rejestrować każdą konwersację codemate jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja codemate będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować codemate tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować codemate tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować codemate tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem codemate. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/codemate).
:::
...
Aby korzystać z Ollama Cloud z codemate:
...
3. Skopiuj klucz API do użycia w codemate.
...
6. **Ważne**: Przed użyciem modeli chmurowych w codemate musisz pobrać informacje o modelu lokalnie:
...
codemate Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół codemate. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>codemate Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **codemate Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API codemate.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji codemate.
...
5. Możesz je także dostosować za pomocą konfiguracji codemate. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z codemate:
...
Możesz także dostosować modele za pomocą konfiguracji codemate. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z codemate. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `codemate.json` w katalogu projektu:
...
Pola `limit` pozwalają codemate zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `codemate auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację codemate i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji codemate.
