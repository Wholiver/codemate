...
Możesz dodać dodatkowe informacje do codemate za pomocą _Model Context Protocol_, w skrócie MCP. codemate obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [codemate Config](https://codemate.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/codemate`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
codemate automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, codemate:
...
Jeśli serwer wymaga uwierzytelnienia, codemate poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `codemate mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji codemate bezpieczne przechowa tokeny w `~/.local/share/codemate/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w codemate, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie codemate, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć codemate z kontem Sentry.
