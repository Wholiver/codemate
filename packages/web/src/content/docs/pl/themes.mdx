...
Dzięki codemate możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie codemate używa naszego własnego motywu `codemate`.
...
codemate ma kilka wbudowanych motywów.
...
- Chcą, aby codemate pasował do wyglądu ich terminala
...
codemate obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/codemate/themes/*.json` lub `$XDG_CONFIG_HOME/codemate/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.codemate/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.codemate/themes/*.json`
