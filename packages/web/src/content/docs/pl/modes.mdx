...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji codemate.
...
te lub skonfiguruj własne za pomocą konfiguracji codemate.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.codemate/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `codemate.json`:
...
- Globalnie: `~/.config/codemate/modes/`
...
Jeśli nie ma wpływu na temperaturę, codemate stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna codemate, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.codemate/modes/` dla trybów zapisanych dla projektu lub `~/.config/codemate/modes/` dla trybów globalnych:
