...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Codemate posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Codemate.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `CODEMATE_EXPERIMENTAL_LSP_TOOL=true` (lub `CODEMATE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Codemate lub gdy zmienna środowiskowa `CODEMATE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Codemate:
