---
title: プラグイン
description: codemate を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで codemate を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、codemate のデフォルトの動作を変更したりできます。
...
- `.codemate/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/codemate/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/codemate/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/codemate/codemate.json`)
2. プロジェクト設定 (`codemate.json`)
3. グローバルプラグインディレクトリ (`~/.config/codemate/plugins/`)
4. プロジェクトプラグインディレクトリ (`.codemate/plugins/`)
...
codemate は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための codemate SDK クライアント。
...
ここでは、codemate を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://codemate.ai/docs/sdk) を参照してください。
...
codemate が `.env` ファイルを読み取らないようにします。
...
プラグインは codemate にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、codemate が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して codemate で使用できます。
