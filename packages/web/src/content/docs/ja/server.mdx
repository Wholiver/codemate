---
title: サーバー
description: HTTP 経由で codemate サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`codemate serve` コマンドは、codemate クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `codemate.local` |
...
HTTP 基本認証でサーバーを保護するには、`codemate_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `codemate` になるか、`codemate_SERVER_USERNAME` を設定してオーバーライドします。これは、`codemate serve` と `codemate web` の両方に当てはまります。
...
`codemate` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
codemate サーバーを使用して、プログラムで codemate と対話します。
:::

`codemate serve` を実行してスタンドアロンサーバーを起動できます。すでに codemate TUI を実行している場合でも、`codemate serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、codemate [IDE](/docs/ide) プラグインによって使用されます。
