---
title: TUI
description: codemate ターミナルユーザーインターフェイスの使用。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

codemate は、LLM を使用してプロジェクトを作業するための対話型ターミナルインターフェイスまたは TUI を提供します。

codemate を実行すると、現在のディレクトリの TUI が開始されます。
...
codemate TUI を使用する場合、「`/`」に続いてコマンド名を入力すると、アクションをすばやく実行できます。例えば：
...
codemate にプロバイダーを追加します。利用可能なプロバイダーから選択し、その API キーを追加できます。
...
codemate を終了します。 _エイリアス_: `/quit`、`/q`
...
これは、サーバー/ランタイムの動作を構成する `codemate.json` とは別です。
...
カスタム TUI 設定パスをロードするには、`codemate_TUI_CONFIG` を使用します。
