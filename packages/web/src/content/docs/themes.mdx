...
With codemate you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, codemate uses our own `codemate` theme.
...
codemate comes with several built-in themes.
...
- Want codemate to match their terminal's appearance
...
codemate supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/codemate/themes/*.json` or `$XDG_CONFIG_HOME/codemate/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.codemate/themes/*.json`
4. **Current working directory** - Defined in `./.codemate/themes/*.json`
