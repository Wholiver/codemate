---
title: SDK
description: Client JS de type sécurisé pour le serveur codemate.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK codemate JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler codemate par programme.
...
Créez une instance de codemate :

```javascript
import { createCodemate } from "@codemate-ai/sdk"

const { client } = await createCodemate()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `codemate.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${codemate.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de codemate, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@codemate-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*codemate" },
})
