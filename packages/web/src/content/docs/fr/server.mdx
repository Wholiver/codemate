---
title: Serveur
description: Interagissez avec le serveur codemate via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `codemate serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client codemate peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `codemate.local` |
...
Définissez `codemate_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `codemate`, ou définissez `codemate_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `codemate serve` et à `codemate web`.
...
Lorsque vous exécutez `codemate`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur codemate pour interagir avec codemate par programmation.
:::

Cette architecture permet à codemate de prendre en charge plusieurs clients et vous permet d'interagir avec codemate par programme.

Vous pouvez exécuter `codemate serve` pour démarrer un serveur autonome. Si vous avez le
codemate TUI en cours d'exécution, `codemate serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins codemate [IDE](/docs/ide).
...
Le serveur codemate expose les API suivantes.
