...
:::caution
Les modes sont désormais configurés via l'option `agent` dans la configuration codemate.
L’option `mode` est désormais obsolète. [En savoir plus](/docs/agents).
:::

Les modes dans codemate vous permettent de personnaliser le comportement, les outils et les prompts pour différents cas d'utilisation.

Il est livré avec deux modes intégrés : **build** et **plan**. Vous pouvez personnaliser ceux-ci ou configurez les vôtres via la configuration codemate.
...
codemate est livré avec deux modes intégrés.
...
- `edit` - Impossible de modifier les fichiers existants, à l'exception des fichiers situés à `.codemate/plans/*.md` pour détailler le plan lui-même
...
Configurez les modes dans votre fichier de configuration `codemate.json` :
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Donc ça marche pour à la fois la configuration globale codemate et la configuration spécifique au projet.
...
Créez des fichiers de mode dans `.codemate/modes/` pour les modes spécifiques au projet ou `~/.config/codemate/modes/` pour les modes globaux :
