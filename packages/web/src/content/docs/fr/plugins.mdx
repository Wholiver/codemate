---
title: Plugins
description: Écrivez vos propres plugins pour étendre codemate.
---

Les plugins vous permettent d'étendre codemate en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de codemate.
...
- `.codemate/plugins/` - Plugins au niveau du projet
- `~/.config/codemate/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/codemate/node_modules/`.
...
1. Configuration globale (`~/.config/codemate/codemate.json`)
2. Configuration du projet (`codemate.json`)
3. Répertoire global des plugins (`~/.config/codemate/plugins/`)
4. Répertoire des plugins du projet (`.codemate/plugins/`)
...
codemate exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK codemate pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre codemate.
...
:::note
Si vous utilisez l'application de bureau codemate, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez codemate de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à codemate :
...
L'assistant `tool` crée un outil personnalisé que codemate peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour codemate aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://codemate.ai/docs/sdk) pour plus de détails.
