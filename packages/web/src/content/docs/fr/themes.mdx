...
Avec codemate, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, codemate utilise notre propre thème `codemate`.
...
codemate est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que codemate corresponde à l'apparence de leur terminal
...
codemate prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/codemate/themes/*.json` ou `$XDG_CONFIG_HOME/codemate/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.codemate/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.codemate/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/codemate/themes
vim ~/.config/codemate/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .codemate/themes
vim .codemate/themes/my-theme.json
```
...
{
  "$schema": "https://codemate.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
