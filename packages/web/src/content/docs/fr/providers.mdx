---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Codemate.
---

import config from "../../../../config.mjs"
export const console = config.console

Codemate utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Codemate.
...
en `~/.local/share/codemate/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Codemate
configuration.
...
Codemate Zen est une liste de modèles fournis par l'équipe Codemate qui ont été
testé et vérifié pour fonctionner correctement avec Codemate. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Codemate Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez codemate et dirigez-vous vers [codemate.ai/auth](https://codemate.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Codemate et son utilisation est totalement facultative.

---

## Codemate Go

Codemate Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Codemate qui ont été
testé et vérifié pour fonctionner correctement avec Codemate.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `Codemate Go` et rendez-vous sur [codemate.ai/auth](https://codemate.ai/zen).
...
Pour utiliser Amazon Bedrock avec Codemate :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de codemate :

```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY codemate

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile codemate

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX codemate
```
...
Pour une configuration spécifique au projet ou persistante, utilisez `codemate.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Codemate n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer codemate pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).

```json title="codemate.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://codemate.ai/config.json",
  "provider": {
    "atomic-chat": {
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que codemate fonctionne correctement.
   :::
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX codemate
   ```
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX codemate
   ```
...
Vous pouvez également ajouter des modèles via votre configuration codemate.
...
pour stocker le jeton dans le stockage d'authentification codemate.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Codemate utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Codemate
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `codemate.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [codemate-gitlab-auth](https://www.npmjs.com/package/codemate-gitlab-auth).

##### Configuration

Personnalisez via `codemate.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec codemate :
...
Pour utiliser Google Vertex AI avec Codemate :
...
Définissez-les lors de l'exécution de codemate.

```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id codemate
```
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via codemate, vous pouvez toujours le configurer vous-même.
...
```jsonc title="~/.config/codemate/codemate.jsonc"
{
  "$schema": "https://codemate.ai/config.json",
  "provider": {
    "helicone": {
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [codemate-helicone-session](https://github.com/H2Shami/codemate-helicone-session) pour enregistrer automatiquement chaque conversation Codemate en tant que session dans Helicone.

```bash
npm install -g codemate-helicone-session
```
...
```json title="codemate.json"
{
  "plugin": ["codemate-helicone-session"]
}
```

Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Codemate répertoriée comme une session distincte.
...
Vous pouvez configurer codemate pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)

```json title="codemate.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://codemate.ai/config.json",
  "provider": {
    "llama.cpp": {
...
Vous pouvez configurer codemate pour utiliser des modèles locaux via LM Studio.

```json title="codemate.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://codemate.ai/config.json",
  "provider": {
    "lmstudio": {
...
Vous pouvez configurer codemate pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour Codemate. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/codemate) pour plus de détails.
:::

```json title="codemate.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://codemate.ai/config.json",
  "provider": {
    "ollama": {
...
Pour utiliser Ollama Cloud avec Codemate :
...
3. Copiez la clé API à utiliser dans Codemate.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Codemate, vous devez extraire les informations du modèle localement :
...
Codemate Zen est une liste de modèles testés et vérifiés fournie par l'équipe Codemate. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Codemate Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Codemate Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Codemate API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration codemate.
...
5. Vous pouvez également les personnaliser via votre configuration codemate. Voici un exemple de spécification d'un fournisseur
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' codemate
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group codemate
   ```
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Codemate :
...
Vous pouvez également personnaliser les modèles via votre configuration codemate. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec codemate. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
3. Entrez votre clé API pour le fournisseur.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in codemate.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Créez ou mettez à jour votre fichier `codemate.json` dans le répertoire de votre projet :

   ```json title="codemate.json" ""myprovider"" {5-15}
   {
     "$schema": "https://codemate.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="codemate.json" {9,11,17-20}
{
  "$schema": "https://codemate.ai/config.json",
  "provider": {
    "myprovider": {
...
Les champs `limit` permettent à Codemate de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `codemate auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration codemate et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration codemate.
