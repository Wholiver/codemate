...
Vous pouvez ajouter des outils externes à codemate à l'aide du _Model Context Protocol_ ou MCP. codemate prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [codemate Config](https://codemate.ai/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Les organisations peuvent fournir des serveurs MCP par défaut via leur point de terminaison `.well-known/codemate`. Ces serveurs peuvent être désactivés par défaut, permettant aux utilisateurs de choisir ceux dont ils ont besoin.
...
codemate gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, codemate :
...
Si le serveur nécessite une authentification, codemate vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `codemate mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, codemate stockera les jetons en toute sécurité dans `~/.local/share/codemate/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans codemate, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration codemate comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter codemate à votre compte Sentry.
