...
Du kan legge til eksterne verktøy i codemate ved å bruke _Model Context Protocol_ eller MCP. codemate støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [codemate Config](https://codemate.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="codemate.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/codemate`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
codemate håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil codemate:
...
Hvis serveren krever autentisering, vil codemate be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `codemate mcp auth <server-name>`.
...
```bash
codemate mcp auth my-oauth-server
```
...
```bash
codemate mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil codemate lagre tokens sikkert i `~/.local/share/codemate/mcp-auth.json`.
...
codemate mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i codemate, sammen med innebygde verktøy. Så du kan administrere dem gjennom codemate-konfigurasjonen som alle andre verktøy.
...
```json title="codemate.json" {11, 14-18}
{
  "$schema": "https://codemate.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
codemate mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble codemate til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver codemate-samtale oppført som en separat økt.
