---
title: Tjener
description: Samhandle med Codemate-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `codemate serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Codemate-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `codemate.local` |
...
Sett `CODEMATE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `codemate` som standard, eller sett `CODEMATE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `codemate serve` og `codemate web`.
...
Når du kjører `codemate` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Codemate-serveren til å samhandle med Codemate programmatisk.
:::

Denne arkitekturen lar Codemate støtte flere klienter og lar deg samhandle med Codemate programmatisk.

Du kan kjøre `codemate serve` for å starte en frittstående server. Hvis du har
Codemate TUI kjører, vil `codemate serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Codemate [IDE](/docs/ide) plugins.
...
codemate-serveren viser følgende APIer.
