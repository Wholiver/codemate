...
:::caution
Moduser konfigureres nå gjennom alternativet `agent` i Codemate-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i Codemate lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom Codemate-konfigurasjonen.
...
Codemate kommer med to innebygde moduser.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.codemate/plans/*.md` for å detaljere selve planen
...
Konfigurer moduser i din `codemate.json` konfigurasjonsfil:
...
- Globalt: `~/.config/codemate/modes/`
- Prosjekt: `.codemate/modes/`
...
Hvis ingen temperatur er spesifisert, bruker Codemate modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Codemate-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Opprett modusfiler i `.codemate/modes/` for prosjektspesifikke moduser eller `~/.config/codemate/modes/` for globale moduser:
