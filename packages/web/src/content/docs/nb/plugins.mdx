...
description: Skriv dine egne programtillegg for å utvide codemate.
---

Plugins lar deg utvide codemate ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til codemate.
...
- `.codemate/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/codemate/node_modules/`.
...
1. Global konfigurasjon (`~/.config/codemate/codemate.json`)
2. Prosjektkonfigurasjon (`codemate.json`)
3. Global plugin-katalog (`~/.config/codemate/plugins/`)
4. Prosjektpluginkatalog (`.codemate/plugins/`)
...
codemate kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En codemate SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide codemate.
...
:::note
Hvis du bruker codemate-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre codemate fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til codemate:
...
`tool`-hjelperen lager et tilpasset verktøy som codemate kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for codemate sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://codemate.ai/docs/sdk) for detaljer.
