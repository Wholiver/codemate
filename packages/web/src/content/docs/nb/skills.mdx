...
Agentferdigheter lar codemate oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
codemate søker etter disse stedene:

- Prosjektkonfigurasjon: `.codemate/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/codemate/skills/<name>/SKILL.md`
...
For prosjektlokale stier går codemate opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.codemate/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/codemate/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.codemate/skills/git-release/SKILL.md` slik:
...
codemate viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `codemate.json`:
...
**For innebygde agenter** (i `codemate.json`):
