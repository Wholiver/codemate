---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Codemate.
---

import config from "../../../../config.mjs"
export const console = config.console

Codemate bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Codemate-konfigurasjonen.
...
i `~/.local/share/codemate/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Codemate
konfig.
...
Codemate Zen er en liste over modeller levert av Codemate-teamet som har vært
testet og verifisert for å fungere godt med Codemate. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Codemate Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Codemate Zen` og gå til [codemate.ai/auth](https://codemate.ai/zen).
...
Det fungerer som alle andre leverandører i Codemate og er helt valgfritt å bruke.

---

## Codemate Go

Codemate Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Codemate-teamet som har vært
testet og verifisert for å fungere godt med Codemate.

1. Kjør kommandoen `/connect` i TUI, velg `Codemate Go`, og gå til [codemate.ai/auth](https://codemate.ai/zen).
...
Slik bruker du Amazon Bedrock med Codemate:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Codemate:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `codemate.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i codemate støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere codemate til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Codemate skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Codemate-konfigurasjonen.
...
for å lagre token i Codemate auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Codemate bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Codemate
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`codemate.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [codemate-gitlab-auth](https://www.npmjs.com/package/codemate-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `codemate.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Codemate:
...
Slik bruker du Google Vertex AI med Codemate:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Codemate.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Codemate, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [codemate-helicone-session](https://github.com/H2Shami/codemate-helicone-session) plugin for automatisk å logge hver Codemate-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Codemate-samtale oppført som en separat økt.
...
Du kan konfigurere Codemate for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Codemate for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Codemate for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Codemate. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/codemate) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Codemate:
...
3. Kopier API-nøkkelen for bruk i Codemate.
...
6. **Viktig**: Før du bruker skymodeller i Codemate, må du hente modellinformasjonen lokalt:
...
Codemate Zen er en liste over testede og verifiserte modeller levert av Codemate-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Codemate Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Codemate Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Codemate API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Codemate-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Codemate-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Codemate:
...
Du kan også tilpasse modeller gjennom Codemate-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Codemate. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `codemate.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Codemate forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `codemate auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Codemate-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Codemate-konfigurasjonen.
