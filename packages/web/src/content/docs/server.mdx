---
title: Server
description: Interact with codemate server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `codemate serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an codemate client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `codemate.local` |
...
Set `codemate_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `codemate`, or set `codemate_SERVER_USERNAME` to override it. This applies to both `codemate serve` and `codemate web`.
...
When you run `codemate` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the codemate server to interact with codemate programmatically.
:::

This architecture lets codemate support multiple clients and allows you to interact with codemate programmatically.

You can run `codemate serve` to start a standalone server. If you have the
codemate TUI running, `codemate serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the codemate [IDE](/docs/ide) plugins.
...
The codemate server exposes the following APIs.
