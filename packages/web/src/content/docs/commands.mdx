...
Create `.codemate/commands/test.md`:
...
You can add custom commands through the codemate config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your codemate [config](/docs/config):
...
codemate includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
