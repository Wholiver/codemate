...
codemate bringt eingebaute Tools mit und laesst sich ueber [Custom Tools](/docs/custom-tools) oder [MCP-Server](/docs/mcp-servers) erweitern.
...
Hier sind alle in codemate verfuegbaren eingebauten Tools.
...
:::note
Dieses Tool ist nur verfuegbar, wenn `codemate_EXPERIMENTAL_LSP_TOOL=true` (oder `codemate_EXPERIMENTAL=true`) gesetzt ist.
:::
...
:::note
Dieses Tool ist nur verfuegbar, wenn der codemate-Provider genutzt wird oder die Umgebungsvariable `codemate_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.

Zum Aktivieren beim Start von codemate:
