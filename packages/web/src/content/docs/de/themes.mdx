...
Mit codemate kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet codemate das `codemate`-Theme.
...
codemate liefert mehrere eingebaute Themes mit.
...
- codemate optisch ans Terminal anpassen wollen
...
codemate unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/codemate/themes/*.json` oder `$XDG_CONFIG_HOME/codemate/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.codemate/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.codemate/themes/*.json`
