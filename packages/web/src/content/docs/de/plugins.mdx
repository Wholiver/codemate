---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Codemate zu erweitern.
---

Mit Plugins können Sie Codemate erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Codemate zu ändern.
...
- `.codemate/plugins/` – Plugins auf Projektebene
- `~/.config/codemate/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/codemate/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/codemate/codemate.json`)
2. Projektkonfiguration (`codemate.json`)
3. Globales Plugin-Verzeichnis (`~/.config/codemate/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.codemate/plugins/`)
...
Codemate führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Codemate-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Codemate erweitern können.
...
:::note
Wenn Sie die Codemate-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Codemate `.env`-Dateien liest:
...
Plugins können Codemate auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Codemate aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Codemate zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://codemate.ai/docs/sdk).
