...
Agent Skills erlauben Codemate, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Codemate sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Codemate vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.codemate/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/codemate/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.codemate/skills/git-release/SKILL.md` zum Beispiel so:
...
Codemate listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `codemate.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `codemate.json`):
