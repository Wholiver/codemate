---
title: Regeln
description: Lege eigene Anweisungen fuer codemate fest.
---

Du kannst codemate ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in codemate den Befehl `/init` aus.
...
So kann codemate sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht codemate sie zu erweitern.
...
codemate kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/codemate/AGENTS.md` speichern.
Diese gelten in allen codemate-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Codemate dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/codemate/AGENTS.md` existiert)
...
Beim Start sucht codemate in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/codemate/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `codemate.json` oder in `~/.config/codemate/codemate.json` angeben.
...
codemate parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit codemate.json

Empfohlen ist das `instructions`-Feld in der `codemate.json`:
...
Du kannst codemate explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `codemate.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
