...
Wenn Codemate Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\codemate\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `codemate --log-level DEBUG`.

---

## Speicher

codemate speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/codemate/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\codemate`
...
Codemate Desktop startet im Hintergrund einen lokalen Codemate-Server (`codemate-cli`-Sidecar).
...
- Nur macOS: `Codemate`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/codemate/codemate.jsonc` (oder `~/.config/codemate/codemate.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/codemate/codemate.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\codemate\codemate.jsonc`
...
Codemate kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/codemate/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\codemate\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Codemate ihn neu aufbauen kann.

1. Beende Codemate Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/codemate`
- **Linux**: loesche `~/.cache/codemate` (oder fuehre aus: `rm -rf ~/.cache/codemate`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\codemate`

3. Starte Codemate Desktop neu.

---

### Server-Verbindungsprobleme beheben

Codemate Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `codemate.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `CODEMATE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `CODEMATE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Codemate Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Codemate-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Codemate Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Codemate in den OS-Einstellungen aktiviert sind, und
...
1. Beende Codemate Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Codemate Desktop):

- `codemate.settings.dat` (desktop default server URL)
- `codemate.global.dat` und `codemate.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Codemate hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `codemate models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\codemate`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. codemate installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\codemate`

2. Starte codemate neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt codemate, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
codemate erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
