---
title: LSP Server
description: codemate lässt sich in Ihren LSP-Server integrieren.
---

codemate lässt sich in Ihr Language Server Protocol (LSP) integrieren, um die Interaktion von LLM mit Ihrer Codebasis zu unterstützen. Es verwendet Diagnosefunktionen, um dem LLM Feedback zu geben.

---

## Integriert

codemate verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `codemate_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn codemate eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer codemate-Konfiguration anpassen.
