---
title: Server
description: Nutze den codemate-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `codemate serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein codemate-Client nutzen kann.
...
Setze `CODEMATE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `codemate`, kann aber mit `CODEMATE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `codemate serve` und `codemate web`.
...
Wenn du `codemate` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den codemate-Server, um codemate programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `codemate serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `codemate serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Codemate-[IDE](/docs/ide)-Plugins.
...
Der codemate-Server stellt folgende APIs bereit.
