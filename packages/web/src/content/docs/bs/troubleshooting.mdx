...
Da biste otklonili probleme s Codemate, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\codemate\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `codemate --log-level DEBUG`.

---

## Pohrana

Codemate pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/codemate/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\codemate`
...
Codemate Desktop pokreće lokalni Codemate server (`codemate-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Codemate` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/codemate/codemate.jsonc` (ili `~/.config/codemate/codemate.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/codemate/codemate.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\codemate\codemate.jsonc`
...
Codemate također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/codemate/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\codemate\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Codemate mogao ponovo izgraditi.

1. Potpuno zatvorite Codemate Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/codemate`
- **Linux**: obrišite `~/.cache/codemate` (ili pokrenite `rm -rf ~/.cache/codemate`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\codemate`

3. Ponovo pokrenite Codemate Desktop.

---

### Rješavanje problema sa vezom na serveru

Codemate Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `codemate.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `CODEMATE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `CODEMATE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Codemate Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Codemate karakteristikama.

---

### Obavještenja se ne prikazuju

Codemate Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Codemate u postavkama vašeg OS-a, i
...
1. Zatvorite Codemate Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Codemate Desktop aplikacije):

- `codemate.settings.dat` (URL zadanog servera za desktop)
- `codemate.global.dat` i `codemate.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `codemate models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\codemate`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Codemate dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\codemate`

2. Ponovo pokrenite Codemate da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Codemate da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Codemate će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
