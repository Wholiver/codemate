...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. codemate dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u codemate.
...
:::note
Ovaj alat je dostupan samo kada je `codemate_EXPERIMENTAL_LSP_TOOL=true` (ili `codemate_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz codemate provajdera ili kada je varijabla `codemate_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju codemate:
