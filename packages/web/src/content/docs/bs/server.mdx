---
title: Server
description: Komunicirajte s codemate serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `codemate serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji codemate klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `codemate.local` |
...
Postavite `codemate_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `codemate`, ili postavite `codemate_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `codemate serve` i za `codemate web`.
...
Kada pokrenete `codemate`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite codemate server za programsku interakciju sa codemate-om.
:::

Ova arhitektura omogucava codemate podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `codemate serve` da startate standalone server. Ako je codemate TUI vec pokrenut, `codemate serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste codemate [IDE](/docs/ide) pluginovi.
...
codemate server izlaže sljedece API-je.
