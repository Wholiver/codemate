---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u codemate.
---

import config from "../../../../config.mjs"
export const console = config.console

codemate koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj codemate konfiguraciji.
...
u `~/.local/share/codemate/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem codemate
config.
...
codemate Zen je lista modela koje je obezbedio codemate tim koji su bili
testirano i potvrđeno da dobro radi sa codemate. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa codemate Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite codemate i idite na [codemate.ai/auth](https://codemate.ai/auth).
...
Radi kao i svaki drugi provajder u codemate i potpuno je opcionalan za korištenje.

---

## codemate Go

codemate Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža codemate tim i koji su testirani i verificirani da dobro rade s codemate-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `codemate Go` i idite na [codemate.ai/auth](https://codemate.ai/zen).
...
Da biste koristili Amazon Bedrock s codemate:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete codemate:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `codemate.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u codemate.
:::
...
Možete konfigurirati codemate za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi codemate ispravno radio.
   :::
...
Također možete dodati modele kroz svoju codemate konfiguraciju.
...
da pohrani token u codemate auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
codemate koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate codemate
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`codemate.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [codemate-gitlab-auth](https://www.npmjs.com/package/codemate-gitlab-auth).

##### Konfiguracija

Prilagodite putem `codemate.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s codemate:
...
Za korištenje Google Vertex AI s codemate:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće codemate.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem codematea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [codemate-helicone-session](https://github.com/H2Shami/codemate-helicone-session) da automatski prijavite svaki codemate razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki codemate razgovor naveden kao zasebna sesija.
...
Možete konfigurirati codemate za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati codemate za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati codemate za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za codemate. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/codemate) za detalje.
:::
...
Da biste koristili Ollama Cloud s codemate:
...
3. Kopirajte API ključ za korištenje u codemate.
...
6. **Važno**: Prije upotrebe modela oblaka u codemate, morate lokalno povući informacije o modelu:
...
codemate Zen je lista testiranih i verifikovanih modela koju je obezbedio codemate tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>codemate Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **codemate Zen**.

```txt
   /connect
```

3. Unesite svoj codemate API ključ.
...
Također možete dodati dodatne modele putem vaše codemate konfiguracije.
...
5. Također ih možete prilagoditi putem vaše codemate konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa codemateom:
...
Također možete prilagoditi modele kroz svoju codemate konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s codemate-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `codemate.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju codemate da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `codemate auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite codemate konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj codemate konfiguraciji.
