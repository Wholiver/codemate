...
Postoje dvije vrste agenata u codemate: primarni agenti i podagenti.
...
codemate dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
codemate dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

codemate dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u codemate-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `codemate.json`:
...
- Globalno: `~/.config/codemate/agents/`
- Po projektu: `.codemate/agents/`

```markdown title="~/.config/codemate/agents/review.md"
---
...
Ako temperatura nije navedena, codemate koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu codemate konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj codemate konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [codemate Zen](/docs/zen), koristili biste `codemate/gpt-5.1-codex` za GPT 5.1 Codex.
...
```json title="codemate.json" {3-6,9-12}
{
  "$schema": "https://codemate.ai/config.json",
  "tools": {
    "write": true,
...
```json title="codemate.json" {3-5,8-10}
{
  "$schema": "https://codemate.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Pokrenite `codemate models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/codemate).
:::
...
```markdown title="~/.config/codemate/agents/docs-writer.md"
---
...
```markdown title="~/.config/codemate/agents/security-auditor.md"
---
