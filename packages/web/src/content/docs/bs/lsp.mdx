---
title: LSP serveri
description: codemate se integriše sa vašim LSP serverima.
---

codemate se integriše sa vašim Language Server Protocol (LSP) serverima kako bi pomogao LLM-u u interakciji s kodnom bazom. Koristi dijagnostiku za slanje povratnih informacija modelu.

## Ugrađeni

codemate dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `codemate_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Možete prilagoditi LSP servere kroz `lsp` odjeljak u vašoj codemate konfiguraciji.
