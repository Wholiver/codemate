...
Agent skills omogucavaju codemate da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
codemate pretrazuje ove lokacije:

- Konfiguracija projekta: `.codemate/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/codemate/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, codemate ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.codemate/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/codemate/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.codemate/skills/git-release/SKILL.md` ovako:
...
codemate navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `codemate.json`:
...
**Za ugradene agente** (u `codemate.json`):
