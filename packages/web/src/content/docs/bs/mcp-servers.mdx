...
Možete dodati vanjske alate u codemate koristeći _Model Context Protocol_, ili MCP. codemate podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [codemate Config](https://codemate.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/codemate`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
codemate automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, codemate će:
...
Ako server zahtijeva autentifikaciju, codemate će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `codemate mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, codemate sigurno čuva tokene u `~/.local/share/codemate/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u codemate, zajedno s ugrađenim alatima. Možete njima upravljati kroz codemate konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete codemate sa vašim Sentry nalogom.
