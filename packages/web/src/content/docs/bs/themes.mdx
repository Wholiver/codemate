...
U Codemate mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Codemate koristi nasu `codemate` temu.
...
Codemate dolazi sa vise ugradenih tema.
...
- Zele da Codemate odgovara izgledu njihovog terminala
...
Codemate podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/codemate/themes/*.json` ili `$XDG_CONFIG_HOME/codemate/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.codemate/themes/*.json`
4. **Trenutni radni direktorij** - `./.codemate/themes/*.json`
