...
ove ili konfigurirajte svoje putem codemate config.
...
codemate dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.codemate/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `codemate.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/codemate/modes/`
- Projekat: `.codemate/modes/`
...
Ako temperatura nije navedena, codemate koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.codemate/modes/` za specifične načine rada ili `~/.config/codemate/modes/` za globalne načine:
