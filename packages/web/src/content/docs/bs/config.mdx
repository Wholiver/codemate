---
title: Konfiguracija
description: Korištenje codemate JSON konfiguracije.
---

Možete konfigurirati codemate koristeći JSON konfiguracijski fajl.

---

## Format

codemate podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/codemate`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/codemate/codemate.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`codemate_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`codemate.json` u projektu) - postavke specifične za projekat
5. **`.codemate` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`codemate_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.codemate` i `~/.config/codemate` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/codemate` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu codemate konfiguraciju u `~/.config/codemate/codemate.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/codemate/tui.json`.
...
Dodajte `codemate.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se codemate pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `codemate_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `codemate_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.codemate` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.codemate`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`codemate.ai/config.json`**](https://codemate.ai/config.json).

TUI konfiguracija koristi [**`codemate.ai/tui.json`**](https://codemate.ai/tui.json).
...
Koristite `codemate_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `codemate.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `codemate serve` i `codemate web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš codemate server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `codemate.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj codemate konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, codemate pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/codemate/agents/` ili `.codemate/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, codemate će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`codemate run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/codemate/commands/` ili `.codemate/commands/`. [Saznajte više ovdje](/docs/commands).
...
codemate će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, codemate **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju codemate sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.codemate/plugins/` ili `~/.config/codemate/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite codemate da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
