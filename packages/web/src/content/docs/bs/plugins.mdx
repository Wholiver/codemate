---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Codemate.
---

Dodaci vam omogućavaju da proširite Codemate spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Codemate.
...
- `.codemate/plugins/` - Dodaci na nivou projekta
- `~/.config/codemate/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/codemate/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/codemate/codemate.json`)
2. Konfiguracija projekta (`codemate.json`)
3. Globalni direktorij dodataka (`~/.config/codemate/plugins/`)
4. Direktorij dodataka projekta (`.codemate/plugins/`)
...
Codemate pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Codemate SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Codemate.
...
Spriječite codemate da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u codemate:
...
Pomoćnik `tool` kreira prilagođeni alat koji codemate može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za codemate zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://codemate.ai/docs/sdk) za detalje.
