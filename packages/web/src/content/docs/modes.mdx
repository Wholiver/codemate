...
:::caution
Modes are now configured through the `agent` option in the codemate config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in codemate allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the codemate config.
...
codemate comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.codemate/plans/*.md` to detail the plan itself
...
Configure modes in your `codemate.json` config file:
...
both the global codemate config and the project specific config.
...
Create mode files in `.codemate/modes/` for project-specific modes or `~/.config/codemate/modes/` for global modes:
