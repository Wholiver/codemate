...
مع codemate يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم codemate سمتنا `codemate`.
...
يأتي codemate مع عدة سمات مدمجة.
...
- يريدون أن يطابق codemate مظهر terminal لديهم
...
يدعم codemate نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/codemate/themes/*.json` أو `$XDG_CONFIG_HOME/codemate/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.codemate/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.codemate/themes/*.json`
