...
:::caution
يتم الآن ضبط الأوضاع عبر خيار `agent` في إعدادات codemate. أصبح خيار
`mode` مُهمَلًا الآن. [اعرف المزيد](/docs/agents).
:::

تتيح لك الأوضاع في codemate تخصيص السلوك والأدوات والمطالبات لحالات استخدام مختلفة.

يأتي مع وضعين مدمجين: **build** و **plan**. يمكنك تخصيصهما أو إعداد
أوضاعك الخاصة عبر إعدادات codemate.
...
- `edit` - لا يمكن تعديل الملفات الموجودة، باستثناء الملفات الموجودة في `.codemate/plans/*.md` لتفصيل الخطة نفسها
...
قم بإعداد الأوضاع في ملف الإعدادات `codemate.json` لديك:
...
- عالمي: `~/.config/codemate/modes/`
- للمشروع: `.codemate/modes/`
...
إذا لم يتم تحديد درجة الحرارة، يستخدم codemate القيم الافتراضية الخاصة بكل نموذج (عادةً 0 لمعظم النماذج و 0.55 لنماذج Qwen).
...
إعدادات codemate العالمية وكذلك إعدادات المشروع.
...
أنشئ ملفات الأوضاع في `.codemate/modes/` لأوضاع خاصة بالمشروع أو في `~/.config/codemate/modes/` لأوضاع عالمية:
