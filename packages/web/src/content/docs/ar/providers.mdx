---
title: المزوّدون
description: استخدام أي موفّر LLM في codemate.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم codemate ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات codemate.
...
في `~/.local/share/codemate/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات codemate.
...
codemate Zen هي قائمة نماذج يوفّرها فريق codemate وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع codemate. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع codemate Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر codemate، ثم انتقل إلى [codemate.ai/auth](https://codemate.ai/auth).
...
يعمل مثل أي مزوّد آخر في codemate واستخدامه اختياري بالكامل.

---

## codemate Go

codemate Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق codemate، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع codemate.

1. شغّل الأمر `/connect` في TUI، واختر `codemate Go`، ثم انتقل إلى [codemate.ai/auth](https://codemate.ai/zen).
...
لاستخدام Amazon Bedrock مع codemate:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل codemate:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `codemate.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل codemate غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين codemate لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل codemate بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات codemate.
...
تخزين الرمز في مساحة تخزين مصادقة codemate.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم codemate نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر codemate
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`codemate.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [codemate-gitlab-auth](https://www.npmjs.com/package/codemate-gitlab-auth).

##### التهيئة

خصّص عبر `codemate.json`:
...
لاستخدام اشتراك GitHub Copilot مع codemate:
...
لاستخدام Google Vertex AI مع codemate:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر codemate، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [codemate-helicone-session](https://github.com/H2Shami/codemate-helicone-session) لتسجيل كل محادثة codemate تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة codemate مدرجة كجلسة مستقلة.
...
يمكنك ضبط codemate لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط codemate لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط codemate لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع codemate. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/codemate) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع codemate:
...
3. انسخ مفتاح API لاستخدامه في codemate.
...
6. **مهم**: قبل استخدام نماذج السحابة في codemate، يجب سحب معلومات النموذج محليا:
...
codemate Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق codemate. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>codemate Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **codemate Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ codemate.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات codemate.
...
5. يمكنك أيضا تخصيصها عبر إعدادات codemate. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع codemate:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات codemate. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع codemate. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `codemate.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ codemate بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `codemate auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات codemate وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات codemate.
