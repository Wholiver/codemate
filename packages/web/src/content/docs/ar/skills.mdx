...
تتيح مهارات الوكيل لـ codemate اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث codemate في هذه المواقع:

- إعدادات المشروع: `.codemate/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/codemate/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج codemate صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.codemate/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/codemate/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.codemate/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض codemate المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `codemate.json`:
...
**للوكلاء المدمجين** (في `codemate.json`):
