---
title: TUI
description: Using the codemate terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

codemate provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running codemate starts the TUI for the current directory.
...
When using the codemate TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to codemate. Allows you to select from available providers and add their API keys.
...
Exit codemate. _Aliases_: `/quit`, `/q`
...
This is separate from `codemate.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long codemate waits after the leader key. Defaults to `2000`.
...
Use `codemate_TUI_CONFIG` to load a custom TUI config path.
