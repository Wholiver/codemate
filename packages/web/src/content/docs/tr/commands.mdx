...
`.codemate/commands/test.md` oluştur:
...
codemate işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

codemate [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/codemate/commands/`
- Proje başına: `.codemate/commands/`
...
codemate, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
