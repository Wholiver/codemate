---
title: Sunucu
description: codemate sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`codemate serve` komutu, codemate istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `codemate.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `CODEMATE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `codemate` değeridir; değiştirmek isterseniz `CODEMATE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `codemate serve` hem de `codemate web` için geçerlidir.
...
`codemate` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
codemate ile programatik etkileşim için codemate sunucusunu kullanın.
:::

Bu mimari, codemate'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`codemate serve` ile bağımsız bir sunucu başlatabilirsiniz. codemate TUI açıksa, `codemate serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum codemate [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
codemate sunucusu aşağıdaki API'leri sunar.
