---
title: Eklentiler
description: codemate'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek codemate'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya codemate'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.codemate/plugins/` - Proje düzeyinde eklentiler
- `~/.config/codemate/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/codemate/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/codemate/codemate.json`)
2. Project config (`codemate.json`)
3. Global eklenti dizini (`~/.config/codemate/plugins/`)
4. Proje eklenti dizini (`.codemate/plugins/`)
...
codemate bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir codemate SDK istemcisi.
...
codemate'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
codemate masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
codemate'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca codemate'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, codemate'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra codemate'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://codemate.ai/docs/sdk) bakın.
