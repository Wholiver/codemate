...
description: codemate, LSP sunucularınızla bütünleşir.
---

codemate, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

codemate, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`codemate_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
codemate bir dosyayı açtığında:
...
LSP sunucularını, codemate yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
