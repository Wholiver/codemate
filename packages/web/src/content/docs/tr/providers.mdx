---
title: Sağlayıcılar
description: codemate'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

codemate, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı codemate yapılandırmanızda yapılandırın.
...
`~/.local/share/codemate/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları codemate'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Codemate Zen, codemate ekibi tarafından sağlanan modellerin bir listesidir.
codemate ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Codemate Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, codemate'u seçin ve [codemate.ai/auth](https://codemate.ai/auth)'ye gidin.
...
codemate'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Codemate Go

Codemate Go, codemate ile iyi çalıştığı test edilmiş ve doğrulanmış, codemate ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Codemate Go`'yu seçin ve [codemate.ai/auth](https://codemate.ai/zen) adresine gidin.
...
Amazon Bedrock'u codemate ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   codemate'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `codemate.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi codemate'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
codemate'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   codemate'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Codemate yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in codemate auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
codemate, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. codemate'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`codemate.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [codemate-gitlab-auth](https://www.npmjs.com/package/codemate-gitlab-auth) ana sayfasında.

##### Yapılandırma

`codemate.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi codemate'la kullanmak için:
...
Google Vertex AI'yi codemate ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları codemate çalıştırılırken ayarlayın.
...
Helicone'un codemate aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her codemate görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [codemate-helicone-session](https://github.com/H2Shami/codemate-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her codemate görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
codemate'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla codemate'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla codemate'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini codemate için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/codemate)'a bakın.
:::
...
Ollama Cloud'u codemate ile kullanmak için:
...
3. codemate'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: codemate'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Codemate Zen, codemate ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Codemate Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Codemate Zen**'i arayın.

   ```txt
   /connect
   ```

3. codemate API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   codemate yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca codemate yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Codemate ile kullanmak için:
...
Modelleri codemate yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı codemate ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `codemate.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları codemate'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `codemate auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için codemate yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, codemate yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
