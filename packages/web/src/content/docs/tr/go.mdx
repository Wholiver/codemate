...
Codemate Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
Codemate Go şu anda beta olarak mevcut.
:::

Go, Codemate'daki diğer sağlayıcılar gibi çalışır. Codemate Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Codemate'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Codemate Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Codemate Go, Codemate'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Codemate Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Codemate Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Codemate Go'ya abone olabilir.
:::
...
Codemate Go aşağıdaki limitleri içerir:
...
Codemate yapılandırmanızdaki [model id](/docs/config/#models) formatı `codemate-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `codemate-go/kimi-k2.6` kullanmalısınız.
...
Codemate Go'yu şu amaçlarla oluşturduk:
...
4. Codemate ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
