...
codemate ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak codemate kendi `codemate` temasını kullanır.
...
codemate birden fazla yerleşik temayla gelir.
...
- codemate'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
codemate, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/codemate/themes/*.json` veya `$XDG_CONFIG_HOME/codemate/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.codemate/themes/*.json`
4. **Geçerli çalışma dizini** - `./.codemate/themes/*.json`
