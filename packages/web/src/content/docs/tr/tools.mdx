...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. codemate yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
codemate içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `codemate_EXPERIMENTAL_LSP_TOOL=true` (veya `codemate_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca codemate provider kullanılırken veya `codemate_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

codemate başlatırken etkinleştirmek için:
