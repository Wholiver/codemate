...
codemate ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\codemate\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `codemate --log-level DEBUG`.

---

## Depolama

codemate, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/codemate/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\codemate` yapistirin
...
codemate Desktop arka planda yerel bir codemate sunucusu (`codemate-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `codemate` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/codemate/codemate.jsonc` (veya `~/.config/codemate/codemate.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/codemate/codemate.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\codemate\codemate.jsonc` yapistirin
...
codemate diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/codemate/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\codemate\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), codemate'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. codemate Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/codemate`
- **Linux**: `~/.cache/codemate` dizinini silin (veya `rm -rf ~/.cache/codemate` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\codemate` yapistirin

3. codemate Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

codemate Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`codemate.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `CODEMATE_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `CODEMATE_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta codemate Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, codemate ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

codemate Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda codemate icin bildirimler etkinse
...
1. codemate Desktop'u kapatin
2. Su dosyalari bulun ve silin (codemate Desktop uygulama veri dizininde yer alirlar):

- `codemate.settings.dat` (desktop varsayilan sunucu URL'si)
- `codemate.global.dat` ve `codemate.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
codemate ile ilgili bir sorun yasiyorsaniz:
...
3. `codemate upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `codemate models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\codemate`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. codemate, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\codemate`

2. En guncel provider paketlerini yeniden kurmak icin codemate'u yeniden baslatin

Bu, codemate'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
codemate, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
