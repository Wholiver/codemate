...
_Model Bağlam Protokolü_ veya MCP kullanarak codemate'a harici araçlar ekleyebilirsiniz. codemate hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [codemate Config](https://codemate.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/codemate` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
codemate, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde codemate şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa codemate, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `codemate mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından codemate, belirteçleri `~/.local/share/codemate/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra codemate'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi codemate yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve codemate'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
