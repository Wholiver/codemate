---
title: TUI
description: Codemate terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Codemate, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

Codemate'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
Codemate TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
Codemate'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
Codemate'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `codemate.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `CODEMATE_TUI_CONFIG` kullanın.
