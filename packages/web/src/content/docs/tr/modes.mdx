...
:::caution
Modlar artık codemate yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

codemate'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya codemate yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
codemate iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.codemate/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`codemate.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, codemate modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global codemate yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.codemate/modes/` veya genel modlar için `~/.config/codemate/modes/`'de mod dosyaları oluşturun:
