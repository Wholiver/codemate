...
Med codemate kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger codemate vårt eget `codemate`-tema.
...
codemate støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/codemate/themes/*.json` eller `$XDG_CONFIG_HOME/codemate/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.codemate/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.codemate/themes/*.json`
