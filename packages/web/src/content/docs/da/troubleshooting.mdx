...
For at fejlfinde problemer med Codemate, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\codemate\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `codemate --log-level DEBUG`.

---

## Lagring

codemate gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/codemate/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\codemate`
...
Codemate Desktop kører en lokal Codemate-server (`codemate-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Codemate`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/codemate/codemate.jsonc` (eller `~/.config/codemate/codemate.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/codemate/codemate.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\codemate\codemate.jsonc`
...
Codemate kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/codemate/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\codemate\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Codemate kan genopbygge den.

1. Afslut Codemate Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/codemate`
- **Linux**: slet `~/.cache/codemate` (eller kør `rm -rf ~/.cache/codemate`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\codemate`

3. Start Codemate Desktop på ny.

---

### Løs problemer med serverforbindelse

Codemate Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `codemate.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `CODEMATE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `CODEMATE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Codemate Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Codemate.

---

### Meddelelser vises ikke

Codemate Desktop viser kun systemvarsler når:

- varsler er aktiveret for Codemate i dine OS-indstillinger, og
...
1. Afslut Codemate Desktop.
2. Find og slet disse filer (de findes i Codemate Desktop-appens datamappe):

- `codemate.settings.dat` (skrivebordsstandardserver URL)
- `codemate.global.dat` og `codemate.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Codemate:
...
For at finde ud af hvilke modeller du har adgang til, kør `codemate models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\codemate`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. codemate installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\codemate`

2. Start codemate på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge codemate til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
codemate vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
