...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i codemate-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i codemate giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem codemate-konfigurationen.
...
codemate leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.codemate/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `codemate.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger codemate modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale codemate-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.codemate/modes/` for projektspecifikke tilstande eller `~/.config/codemate/modes/` for globale tilstande:
