---
title: Plugins
description: Skriv dine egne plugins for at udvide codemate.
---

Plugins giver dig mulighed for at udvide codemate ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre codemates standardadfærd.
...
- `.codemate/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/codemate/node_modules/`.
...
1. Global konfiguration (`~/.config/codemate/codemate.json`)
2. Projektkonfiguration (`codemate.json`)
3. Global plugin-mappe (`~/.config/codemate/plugins/`)
4. Projekt plugin bibliotek (`.codemate/plugins/`)
...
codemate kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En codemate SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide codemate.
...
:::note
Hvis du bruger codemate desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå codemate i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til codemate:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som codemate kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for codemate sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://codemate.ai/docs/sdk) for detaljer.
