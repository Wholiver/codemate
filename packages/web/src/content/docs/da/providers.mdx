---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i codemate.
---

import config from "../../../../config.mjs"
export const console = config.console

codemate bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i codemate-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/codemate/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i codemate-konfigurationen.
...
codemate Zen er en liste over modeller leveret af codemate-teamet, som er blevet testet og verificeret til at fungere godt med codemate. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med codemate Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg codemate og gå til [codemate.ai/auth](https://codemate.ai/auth).
...
Det fungerer som alle andre udbydere i codemate og er helt valgfrit at bruge.

---

## codemate Go

codemate Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af codemate-teamet, som er testet og verificeret til at fungere godt med codemate.

1. Kør kommandoen `/connect` i TUI, vælg `codemate Go`, og gå til [codemate.ai/auth](https://codemate.ai/zen).
...
Sådan bruger du Amazon Bedrock med codemate:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører codemate:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `codemate.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i codemate understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere codemate til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at codemate skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem codemate-konfigurationen.
...
gemme tokenet i codemate auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
codemate bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse codemate
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`codemate.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [codemate-gitlab-auth](https://www.npmjs.com/package/codemate-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `codemate.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med codemate:
...
Sådan bruger du Google Vertex AI med codemate:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører codemate.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem codemate, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [codemate-helicone-session](https://github.com/H2Shami/codemate-helicone-session) plugin for automatisk at logge hver codemate-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver codemate-samtale opført som en separat session.
...
Du kan konfigurere codemate til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere codemate til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere codemate til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for codemate. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/codemate) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med codemate:
...
3. Kopier API-nøglen til brug i codemate.
...
6. **Vigtigt**: Før du bruger skymodeller i codemate, skal du hente modelinformationen lokalt:
...
codemate Zen er en liste over testede og verificerede modeller leveret af codemate-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>codemate Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **codemate Zen**.

   ```txt
   /connect
   ```

3. Indtast codemate API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem codemate-konfigurationen.
...
5. Du kan også tilpasse dem gennem codemate-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med codemate:
...
Du kan også tilpasse modeller gennem codemate-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med codemate. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `codemate.json`-filen i projektkataloget:
...
`limit`-felterne lader codemate forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `codemate auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek codemate-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i codemate-konfigurationen.
