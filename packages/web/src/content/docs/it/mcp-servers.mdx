...
Puoi aggiungere strumenti esterni a codemate usando il _Model Context Protocol_, o MCP. codemate supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [codemate Config](https://codemate.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/codemate`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
codemate gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, codemate:
...
Se il server richiede autenticazione, codemate ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `codemate mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, codemate memorizzera' i token in modo sicuro in `~/.local/share/codemate/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in codemate insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione codemate come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare codemate al tuo account Sentry.
