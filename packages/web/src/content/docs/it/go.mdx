...
Codemate Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

:::note
Codemate Go è attualmente in beta.
:::

Go funziona come qualsiasi altro provider in Codemate. Ti abboni a Codemate Go e
...
Codemate Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

Codemate Go funziona come qualsiasi altro provider in Codemate.

1. Accedi a **<a href={console}>Codemate Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `Codemate Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Codemate Go.
:::
...
Codemate Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Codemate config
utilizza il formato `codemate-go/<model-id>`. Ad esempio, per Kimi K2.6, useresti
`codemate-go/kimi-k2.6` nella tua configurazione.
...
Abbiamo creato Codemate Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Codemate.
