---
title: Server
description: Interagisci con il server codemate via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `codemate serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client codemate.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `codemate.local` |
...
Imposta `codemate_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `codemate`, oppure imposta `codemate_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `codemate serve` sia per `codemate web`.
...
Quando esegui `codemate` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server codemate per interagire con codemate in modo programmatico.
:::

Questa architettura permette a codemate di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `codemate serve` per avviare un server standalone. Se la TUI di codemate e gia in esecuzione, `codemate serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di codemate.
...
Il server codemate espone le seguenti API.
