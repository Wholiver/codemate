...
Crea `.codemate/commands/test.md`:
...
Puoi aggiungere comandi personalizzati tramite la config di codemate o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di codemate:
...
- Globale: `~/.config/codemate/commands/`
- Per progetto: `.codemate/commands/`
...
codemate include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
