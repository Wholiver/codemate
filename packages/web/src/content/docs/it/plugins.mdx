---
title: Plugin
description: Scrivi plugin per estendere codemate.
---

I plugin ti permettono di estendere codemate agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di codemate.
...
- `.codemate/plugins/` - plugin a livello progetto
- `~/.config/codemate/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/codemate/node_modules/`.
...
1. Config globale (`~/.config/codemate/codemate.json`)
2. Config di progetto (`codemate.json`)
3. Directory plugin globale (`~/.config/codemate/plugins/`)
4. Directory plugin di progetto (`.codemate/plugins/`)
...
codemate esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di codemate per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere codemate.
...
:::note
Se usi l'app desktop di codemate, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a codemate di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a codemate:
...
L'helper `tool` crea uno strumento personalizzato che codemate può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in codemate insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://codemate.ai/docs/sdk) per i dettagli.
