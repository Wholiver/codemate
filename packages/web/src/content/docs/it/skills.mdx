...
Le skill degli agenti permettono a codemate di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
codemate cerca in queste posizioni:

- Config di progetto: `.codemate/skills/<name>/SKILL.md`
- Config globale: `~/.config/codemate/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, codemate risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.codemate/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/codemate/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.codemate/skills/git-release/SKILL.md` cosi':
...
codemate elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `codemate.json`:
...
**Per agenti integrati** (in `codemate.json`):
