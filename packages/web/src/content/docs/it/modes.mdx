...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di codemate. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in codemate ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di codemate.
...
codemate include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.codemate/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `codemate.json`:
...
- Globale: `~/.config/codemate/modes/`
- Progetto: `.codemate/modes/`
...
Se non viene specificata alcuna temperatura, codemate usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di codemate sia per quella specifica del progetto.
...
Crea file di modalita in `.codemate/modes/` per modalita specifiche del progetto oppure in `~/.config/codemate/modes/` per modalita globali:
