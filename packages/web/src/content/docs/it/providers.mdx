---
title: Provider
description: Usa qualsiasi provider LLM in codemate.
---

import config from "../../../../config.mjs"
export const console = config.console

codemate usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di codemate.
...
codemate Zen e una lista di modelli forniti dal team codemate, testati e
verificati per funzionare bene con codemate. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da codemate Zen.
:::

1. Esegui `/connect` nella TUI, seleziona codemate e vai su [codemate.ai/auth](https://codemate.ai/auth).
...
Funziona come qualsiasi altro provider in codemate ed e totalmente opzionale.
...
Per usare Amazon Bedrock con codemate:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui codemate:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `codemate.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in codemate non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare codemate per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché codemate funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di codemate.
...
salvare il token nello storage di auth di codemate.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
codemate usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare codemate
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `codemate.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [codemate-gitlab-auth](https://www.npmjs.com/package/codemate-gitlab-auth).

##### Configurazione

Personalizza tramite `codemate.json`:
...
Per usare il tuo abbonamento GitHub Copilot con codemate:
...
Per usare Google Vertex AI con codemate:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui codemate.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da codemate, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [codemate-helicone-session](https://github.com/H2Shami/codemate-helicone-session) per loggare automaticamente ogni conversazione di codemate come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di codemate elencata come sessione separata.
...
Puoi configurare codemate per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare codemate per usare modelli locali tramite LM Studio.
...
Puoi configurare codemate per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per codemate. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/codemate) per dettagli.
:::
...
Per usare Ollama Cloud con codemate:
...
3. Copia la chiave API da usare in codemate.
...
6. **Importante**: Prima di usare modelli cloud in codemate, devi scaricare le informazioni del modello localmente:
...
codemate Zen è una lista di modelli testati e verificati forniti dal team codemate. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>codemate Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **codemate Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di codemate.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di codemate.
...
5. Puoi anche personalizzarli tramite la configurazione di codemate. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con codemate:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di codemate. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con codemate. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `codemate.json` nella directory del progetto:
...
I campi `limit` permettono a codemate di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `codemate auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di codemate e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config codemate.
