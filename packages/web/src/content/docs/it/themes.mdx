...
Con codemate puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, codemate usa il tema `codemate`.
...
codemate include diversi temi integrati.
...
- Vuole che codemate corrisponda all'aspetto del terminale
...
codemate supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/codemate/themes/*.json` o `$XDG_CONFIG_HOME/codemate/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.codemate/themes/*.json`
4. **Directory di lavoro corrente** - in `./.codemate/themes/*.json`
