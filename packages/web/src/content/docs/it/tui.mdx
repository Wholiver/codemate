---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di codemate.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

codemate fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire codemate avvia la TUI per la directory corrente.
...
Quando usi la TUI di codemate, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a codemate. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da codemate. _Alias_: `/quit`, `/q`
...
Questo è separato da `codemate.json`, che configura il comportamento di server/runtime.
...
Usa `codemate_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
