...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@codemate/acme")).toBe("@codemate/acme")
    expect(Npm.sanitize("@codemate/acme@1.0.0")).toBe("@codemate/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/codemate/acme.git"
    const expected = win ? "acme@git+https_//github.com/codemate/acme.git" : spec
