...
async function tmpdir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "codemate-core-test-"))
  return {
    path: dir,
  ...
  describe("Windows-specific", () => {
    fx.effect(
      ...
      Effect.gen(function* () {
        ...
        const out = yield* ChildProcessSpawner.ChildProcessSpawner.use((svc) =>
          svc.string(
            ChildProcess.make("set", ["codemate_TEST_SHELL"], {
              shell: true,
              extendEnv: true,
              env: { codemate_TEST_SHELL: "ok" },
            }),
          ),
        )
        expect(out).toContain("codemate_TEST_SHELL=ok")
      }),
    )
