...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("codemate-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("codemate_lib=debug,codemate_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("codemate_lib=info,codemate_desktop=info,sidecar=info")
        }
    });
