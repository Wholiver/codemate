...
const APP_IDS: Record<string, string> = {
  dev: "ai.codemate.desktop.dev",
  beta: "ai.codemate.desktop.beta",
  prod: "ai.codemate.desktop",
}
const TEST_ONBOARDING = process.env.codemate_TEST_ONBOARDING === "1"
...
const main = Effect.gen(function* () {
  ...
  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.codemate.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `codemate-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "codemate Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("codemate://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("codemate")
  registerRendererProtocol()
  setDockIcon()
  setupAutoUpdater()

  const needsMigration = ((): boolean => {
    if (process.env.codemate_DB === ":memory:") return false

    const xdg = process.env.XDG_DATA_HOME
    const base = xdg && xdg.length > 0 ? xdg : join(homedir(), ".local", "share")
    return !existsSync(join(base, "codemate", "codemate.db"))
  })()
  let overlay: BrowserWindow | null = null

  const port = yield* Effect.gen(function* () {
    const fromEnv = process.env.codemate_PORT
    if (fromEnv) {
