...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.codemate.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `codemate.settings.dat` is special: it maps to the `codemate.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"codemate.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // codemate.settings.dat → the electron settings store ("codemate.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "codemate.global.dat").
  const storeName = filename === "codemate.settings.dat" ? "codemate.settings" : filename
