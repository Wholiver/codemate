...
const deepLinkEvent = "codemate:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__CODEMATE__ ??= {}
  const pending = window.__CODEMATE__.deepLinks ?? []
  window.__CODEMATE__.deepLinks = [...pending, ...urls]
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__CODEMATE__?.wsl) return undefined
    return commands.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__CODEMATE__?.wsl) return result
  ...
  return {
      ...
      await Promise.resolve()
        .then(() => {
          const notification = new Notification(title, {
            body: description ?? "",
            icon: "https://codemate.ai/favicon-96x96-v3.png",
          })
    ...
    getWslEnabled: async () => {
      ...
      return window.__CODEMATE__!.wsl ?? false
    },
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("codemate.global.dat").getItem("language")
