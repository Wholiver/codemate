...
import { createCodemate } from "@codemate-ai/sdk"
...
`)
  ...
  const codemate = await createCodemate({ port: 0 })
  ...
  try {
    ...
    const session = await codemate.client.session.create()
    const result = await codemate.client.session
      .prompt({
