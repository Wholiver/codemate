...
flowchart TD
  ...
  D1 --> D1c[检索模式: keyword / semantic / hybrid]

  D --> D2[Lessons 系统]
  D2 --> D2a[存储: .codemate/lessons.md]
  D2 --> D2b[写入: lesson_write]
  D2 --> D2c[加载: project-lessons]
  ...
  G --> G3[自定义检查: pytest / go test / cargo test]

  G --> H[反馈闭环]
  H --> H1[记录失败与修复]
  H --> H2[回写 lessons 与 memory]
  H --> H3[增强下一次执行质量]
  H --> C
```
